...
# Read-only inventory to spot overlaps and routing pitfalls quickly.
set -euo pipefail
...
api() { curl -sS -H "Authorization: Bearer $CF_API_TOKEN" "https://api.cloudflare.com/client/v4$1"; }

echo "== Access apps on $HOST =="
APPS_JSON="$(api "/accounts/$CF_ACCOUNT_ID/access/apps?per_page=1000")"
echo "$APPS_JSON" | jq -r --arg host "$HOST" '
  .result
  | map(select((.domain==$host) or (.self_hosted_domains // [] | any(.domain==$host))))
  | .[] as $a
  | "APP\t\($a.id)\t\($a.name)\t" +
    (if $a.domain then "domain=\($a.domain)" else "multi" end) + "\tpaths=" +
    ((($a.self_hosted_domains // []) | map(select(.domain==$host) | .path)) | join(","))'

# collect app ids for that host
...
for id in "${APP_IDS[@]}"; do
  ...
  | jq -r '
      .result | sort_by(.precedence) | to_entries[]
...
| jq -r --arg host "$HOST" '
    ...
    | if length == 0 then "NONE"
      else .[] | "ROUTE\t\(.pattern)\tscript=\(.script // "NONE")"
      end'

echo -e "\n== Tunnel published routes (ingress order) =="
ING_CFG="$(api "/accounts/$CF_ACCOUNT_ID/cfd_tunnel/$CF_TUNNEL_ID/configurations" || true)"
if ! echo "$ING_CFG" | jq -e '.success == true' >/dev/null 2>&1; then
  ING_CFG="$(api "/accounts/$CF_ACCOUNT_ID/cfd_tunnel/$CF_TUNNEL_ID/config" || true)"
fi
echo "$ING_CFG" | jq -r '
...
echo -e "\n== Heuristics =="
APPCOUNT=${#APP_IDS[@]}
echo "ACCESS_APPS_FOR_HOST=$APPCOUNT  # expect 2 (Protected + Health)"

# host-scoped ingress list for stronger checks
HOST_ING="$(echo "$ING_CFG" | jq --arg host "$HOST" '
  (.result.config.ingress // .result.ingress)
  | map(select(.hostname == $host))')"
...
# flag any bad path like "*" (missing leading slash)
BAD_PATHS="$(echo "$ING_CFG" | jq -r '
  (.result.config.ingress // .result.ingress)
  | map(select((.hostname!=null) and (.path!=null) and (.path|startswith("/")==false)))
  | .[]? | "\(.hostname)  \(.path)"')"

if [[ -n "${BAD_PATHS:-}" ]]; then
  echo -e "\nWARN: Found ingress paths missing leading \"/\" (should be e.g. \"/*\"):"
  echo "$BAD_PATHS"
fi
