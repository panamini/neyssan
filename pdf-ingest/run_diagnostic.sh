...
# Helper to timestamp lines (works on macOS without ts command)
ts_line() { while IFS= read -r line; do printf '%s %s\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" "$line"; done; }
...
# Run in detached mode but still log output to file
docker-compose up -d db redis
docker-compose logs db redis --tail=50 2>&1 | while IFS= read -r line; do
    echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') $line" | tee -a "${LOG_FILE}"
done
sleep 5
...
PDF_INGEST_DEBUG_LOG="${PDF_INGEST_DEBUG_LOG:-/tmp/pdf_ingest_debug.log}" LLM_MOCK=true docker-compose up -d web worker
docker-compose logs web worker --tail=50 2>&1 | while IFS= read -r line; do
    echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') $line" | tee -a "${LOG_FILE}"
done
sleep 8
...
echo "PARSE HTTP ${PARSE_CODE}" | ts_line >> "${LOG_FILE}"
echo "$PARSE_BODY" | ts_line >> "${LOG_FILE}"
...
echo "CONFIRM SAVE HTTP ${SAVE_CODE}" | ts_line >> "${LOG_FILE}"
echo "$SAVE_BODY" | ts_line >> "${LOG_FILE}"
...
echo "Persisted / placeholder profile id: ${PROFILE_ID}" | ts_line >> "${LOG_FILE}"
...
echo "Refine payload (trimmed):" | ts_line >> "${LOG_FILE}"
echo "$REFINE_PAYLOAD" | jq '.' | ts_line >> "${LOG_FILE}"
...
echo "HTTP ${REF_CODE}" | ts_line >> "${LOG_FILE}"
echo "$REF_BODY" | ts_line >> "${LOG_FILE}"
JOB_ID=$(echo "$REF_BODY" | jq -r '.jobId // .llm_job_id // empty' || echo "")
echo "Job ID: ${JOB_ID}" | ts_line >> "${LOG_FILE}"
...
if [ -n "$JOB_ID" ]; then
  ...
  while [ $ATTEMPT -lt $MAX_ATTEMPTS ]; do
    ...
    echo "poll $ATTEMPT -> $ST" | ts_line >> "${LOG_FILE}"
    STATUS=$ST
    if [ "$ST" = "finished" ] || [ "$ST" = "failed" ]; then
      break
    fi
  done
  echo "Final job status: $STATUS" | ts_line >> "${LOG_FILE}"
else
  echo "No job id available; skipping polling" | ts_line >> "${LOG_FILE}"
fi
...
if [ -n "$JOB_ID" ]; then
  curl -s "http://127.0.0.1:8000/api/v1/llm-history/$JOB_ID" | jq '.' 2>&1 | ts_line >> "${LOG_FILE}" || true
fi

echo "==== PROFILE ROW ====" | tee -a "${LOG_FILE}"
curl -s "http://127.0.0.1:8000/api/v1/profiles/$PROFILE_ID" | jq '.' 2>&1 | ts_line >> "${LOG_FILE}" || true

echo "==== PROFILE LLM HISTORY LIST ====" | tee -a "${LOG_FILE}"
curl -s "http://127.0.0.1:8000/api/v1/profiles/$PROFILE_ID/llm-history" | jq '.' 2>&1 | ts_line >> "${LOG_FILE}" || true

########################################
# Step 8: worker logs
########################################
echo "==== WORKER LOGS (tail 200) ====" | tee -a "${LOG_FILE}"
docker-compose logs worker --tail 200 2>&1 | ts_line >> "${LOG_FILE}" || true
...
docker-compose down 2>&1 | ts_line >> "${LOG_FILE}" || true
