...
CvForge
  -> VerbatiCvPreviewPanel
    -> mapCvDocumentToResumeData(currentCv)
    -> VerbatiResumePreview
      -> ResumePage(variant selected from style preset)
      -> user click produces ResumeLinkIntent
  -> ProfileReviewCard / SectionEditor
    -> resolve canonical section target
    -> open modal or focus inline surface
    -> mutate currentCv
  -> VerbatiCvPreviewPanel remaps currentCv to ResumeData
  -> ResumePage rerenders
```
