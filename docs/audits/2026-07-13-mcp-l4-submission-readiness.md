...
| `search` / `fetch` | Fixed summary-catalog identifiers, titles, categories, fixed source label `twoweeks_safe_summary_catalog`, fixed explanatory text, and fixed catalog URLs. Both tools mirror their fixed payload in `structuredContent` and serialized `content[0].text`. | User documents, prompts, provider output, account data, arbitrary URLs. |
...
| Tool inventory and annotations | The L2 CI test proves that the smoke validator rejects fixture inventory drift. The active production projection is separately covered by the authenticated route-adapter `tools/list` test, but that production-backed test is not yet part of the L2 CI job. | Add the production projection test to CI after the upstream CI ownership/base is stable, then confirm the deployed endpoint scans to the same exact inventory. |
