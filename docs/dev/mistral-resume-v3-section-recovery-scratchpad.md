...
- explicit `Experience` / `Work History` section detection
...
- [cv_parser_service/mistral_ocr.py](/Volumes/video/kay/app/pouraurelien/save/implementation_UI/neyssan/cv_parser_service/mistral_ocr.py)
...
Current experience aliases include:
...
Compound heading support is intentionally narrow:

- headings are split only on safe delimiters such as `&`, `/`, `|`, `,`, ` y `, ` et `, ` und `, ` e `
- the classifier accepts a compound heading only when all recognized segments resolve to the same section family
...
Examples that are intentionally not accepted as a single-family heading:
...
Experience recovery is section-local and only mines the explicit OCR markdown section body.

Code path:

- `_extract_explicit_experience_from_sections(...)`
- `_parse_experience_header_line(...)`
- `_parse_experience_date_line(...)`

Current supported recovery shapes include:

- markdown subheadings like `## Company - Position`
- inline headers like `Position at Company`
- inline headers like `Company | Position | Date Range`
- role metadata lines like `Location • 08/2010 - Current`

Important constraints:

- only the matched experience/work-history section body is used
- nested markdown headings inside the section are preserved as entry headers
- recovery preserves explicit bullets as `responsibilityBullets`
- recovery preserves inline prose as `description`
- recovery only accepts entries that survive re-normalization with real experience content
...
- if explicit `experience` section exists and normalized `experience` is empty, reject
...
- `_validated_recovered_experience(...)`
...
- alias classification tests for multilingual experience-family headings
- nested markdown heading preservation inside explicit experience/work-history sections
- deterministic recovery from `WORK HISTORY`
- deterministic recovery from localized headings such as:
  - `RELEVANT EXPERIENCE`
  - `EXPERIÊNCIA PROFISSIONAL`
  - `EXPÉRIENCE PROFESSIONNELLE`

The parser suite currently verifies these paths in:

- [cv_parser_service/tests/test_mistral_resume_v3_pipeline.py](/Volumes/video/kay/app/pouraurelien/save/implementation_UI/neyssan/cv_parser_service/tests/test_mistral_resume_v3_pipeline.py)

The shipped regression baseline for this work is:

- `python3 -m pytest cv_parser_service/tests/test_mistral_resume_v3_pipeline.py`
- expected result at implementation time: `88 passed`
...
- [test_mistral_resume_v3_pipeline.py](/Volumes/video/kay/app/pouraurelien/save/implementation_UI/neyssan/cv_parser_service/tests/test_mistral_resume_v3_pipeline.py)
