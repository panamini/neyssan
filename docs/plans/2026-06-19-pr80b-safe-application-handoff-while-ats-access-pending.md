...
## Maintainer decisions after PR80B preflight

These decisions are controlling for PR80B and supersede older proposal wording below wherever the older wording is broader.

1. UI scope
- PR80B may modify the existing `/jobs/:jobId` workspace only.
- No new route or generic UI framework.
- A small dedicated handoff component is allowed.

2. Package eligibility
- `ready_for_review` is not final approval.
- It only permits preparing the handoff preview.
- Existing artifact approval/freshness requirements must pass.
- The exact PR80B confirmation authorizes copy/download/open.

3. Ownership
- derive identity from `ctx.auth`;
- map it to the internal user profile;
- verify package, job, application context and artifacts belong together;
- never authorize from a caller-supplied `userId` or `clerkId`.

4. Destination
- use only the approved `job.applicationUrl`;
- no `sourceUrl` fallback;
- no caller-supplied URL;
- missing `applicationUrl` produces a blocked handoff;
- HTTPS only;
- no server fetch or DNS resolution.
...
`not_started` is UI-derived and is not persisted.

`provider_verified_submitted` and `provider_receipt_verified` do not exist in PR80B writable validators.

6. Evidence and audit
- no generic `evidenceLevels` array;
- evidence belongs to exact append-only events;
- allowed evidence:
  - `twoweeks_prepared`
  - `user_interaction_observed`
  - `user_reported`
- no provider receipt evidence;
- no free-text outcome note in V1.

Use precise event names:
...
7. Data minimization
- do not persist `actorClerkId`;
- do not persist answer text;
- do not persist CV or cover-letter text;
- do not persist document bytes;
- do not persist the full destination URL;
- persist internal refs, safe URL origin/hostname, URL hash, manifest digest, state, evidence metadata and timestamps only.

8. PR80A separation
- never reserve or dispatch a PR80A `liveExternalActionExecution` for manual handoff;
- never finalize manual handoff as provider submitted;
- PR80A utilities may be reused only when their semantics are neutral and truthful.

9. UI gestures
- copy uses a direct human click;
- no clipboard reads;
- no automatic copy;
- sensitive answers copied individually;
- download is direct user initiated;
- open uses a direct human click;
- opening a destination proves only that navigation was requested.

10. Feature flag
- separate server-owned flag: `TWOWEEKS_MANUAL_APPLICATION_HANDOFF_ENABLED`;
- false by default;
- malformed and missing values mean false;
- it does not enable live ATS actions.

11. Explicit unlock
PR80B explicitly unlocks only:
- authenticated owner-scoped Convex reads/writes for manual handoff records;
- a narrowly scoped `/jobs/:jobId` handoff UI;
- browser-local copy/download/open actions initiated directly by the user.

It does not unlock:
- OAuth runtime changes;
- external HTTP;
- provider transport;
