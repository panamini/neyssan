...
2. Clerk account deletion is globally bounded and resumable across many Profiles and Jobs, hides Profile and Job summaries immediately, and removes all owned Profiles, Jobs, catalog projections, and backfill state before completion.
...
- Final diff contains exactly the six allowlisted paths.
- Review cycle 1/2 and Fallow are read-only. At most one in-scope review-fix cycle is allowed before exact-head publication.
...
- The terminal tombstone now makes account recreation a no-op, and every deletion replay rechecks bounded owned rows even after `done`. No further fix cycle is permitted.
