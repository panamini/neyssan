...
## Change Contract v9 — commuting materialization and complete bounded lists

- Baseline: exact PR head `1f5f01f51bfc6fff5d67762ff6d5f7fca313cc86` on `codex/neyssan-stabilization-read-models`; clean worktree; PR base remains `21fba4869740938087ca4b44fa18f62b3b12d5c0`.
- Authorization/status: controller's exact-head four-finding directive; `AUTHORIZED_TO_IMPLEMENT`; risk `MEDIUM` (additive schema plus bounded projection concurrency and pagination behavior).
- Atomic outcome: make proposal materialization idempotent under scan/live-write interleavings, make client orchestration progress-aware rather than attempt-capped, paginate archived Jobs without truncation, and make list/detail structured-shadow selection deterministic and identical.
- Invariants: per-proposal desired state is materialized once and stale/repeated observations are no-ops; create/update/reassign/delete commute with bounded backfill and preserve exact count/latest activity beyond 100; account orchestration continues while persisted progress changes and fails only after repeated identical progress; active and archived first pages remain 36 and expose accessible continuation; both list and detail inspect the same bounded eligible-shadow window and prefer the newest valid non-fallback evidence.
- Allowed: `my-app/convex/schema.ts`, `my-app/convex/lib/jobCatalog.ts`, `my-app/convex/lib/jobs/structuredMatchRead.ts`, `my-app/convex/jobsPublic.ts`, proposal write callers only if the shared materialization contract requires it, `my-app/src/hooks/useJobsQuery.ts`, `my-app/src/components/jobs/JobsWorkspace.tsx`, focused existing Jobs/proposal tests, and this contract. Conditional: Jobs list/page CSS or component tests only if the existing accessible Load more surface cannot be reused unchanged. Forbidden: PR2 CV hydration, proposal lineage expansion, production/shared migrations, deployment, merge, parent artifact, force push, or a second top-level review request.
- Design: add an indexed per-proposal materialization ledger and versioned aggregate initialization; derive deltas from ledger state rather than caller snapshots, with monotonic proposal activity/tombstones; remove replay-unsafe aggregate resets. Return persisted phase/cursor progress from each finite backfill mutation and detect repeated no-progress client-side without an absolute call ceiling. Convert archived query/hook to the same cursor pagination contract and route the existing Load more surface by current view. Export one structured-shadow window constant and use it in both catalog and detail selection.
- Required evidence: RED then green for repeated/interleaved proposal materialization, ordinary edit/reassign/delete and >100 exactness; >800 changing progress responses complete while repeated identical progress fails closed; >80 archived rows paginate in activity order; valid shadow followed by invalid/fallback keeps list/detail agreement. Then one crash-safe focused batch, targeted type/lint/diff, Changeset self-review, Fallow read-only, exact full/staged/sensitive review, one commit/push to PR #374, and exact-head replies/resolution for only the four v9 threads.
- Recovery: additive projection state can be reverted by a later authorized revert; no production data operation is executed in this task.

### V9 evidence ledger

- Focused affected batch: PASS, 196/196 tests across proposal ownership/materialization, job catalog, Jobs backend/read-model scale, profile catalog, progress-aware hook, and Jobs UI. It includes >800 changing progress responses reaching ready, repeated identical progress failing closed, 90 archived rows across three 36-row pages, accessible archived Load more, repeated/stale proposal replay, >100 exact proposal activity, and valid-shadow then invalid/fallback preservation.
- Isolated self-hosted execution: PASS on the owned temporary backend at 3230/3231 with the existing synthetic 100-heavy-profile / 500-Job / 198-proposal / 500-shadow database. Version-4 finite materialization completed, then an authenticated metadata invalidation was interleaved with one backfill page and an ordinary proposal edit before completion. Final proof reported 66 advancing calls, first page 36 with continuation, target proposal count exactly 148, visible tier `unknown`, and archived top activity `20000`. The log interval from the valid profile write through final verification contains no memory warning or unexpected function failure. Earlier overlapping-harness OCC retries and an obsolete archived-query argument error are setup/harness events outside that final interval, not product-pass evidence.
- Proof classes: the live run covers schema deployment, finite replay/live-write interleaving, exact materialized count, bounded active/archived reads, and shadow-derived visible tier. The synthetic >800 and >80 cases remain deterministic focused behavioral tests because this owned database has about 100 profiles and only a small archived subset; no stronger runtime claim is made for those two cardinalities.
- Static/final review: TypeScript reports no errors; targeted source lint is clean (test files are repository-ignored and reported only as ignore warnings); `git diff --check` passes. Fallow `audit --base 1f5f01f5...` again exits 2 because it cannot create its temporary worktree, so no audit-pass claim is made. Read-only `fallow health` completed and reported repository-wide complexity hotspots including the pre-existing `JobsPageContent`; no automated fix was applied and no output identified the small v9 ledger/progress helpers as a new defect.
