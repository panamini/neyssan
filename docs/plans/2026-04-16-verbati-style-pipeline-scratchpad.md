...
```ts
if (hasUserEditedStyle && workspaceStyle) return { style: workspaceStyle, source: "custom" };
if (!hasUserEditedStyle && isCvAttached && cvStyle) return { style: cvStyle, source: "cv" };
if (metadataStyle) return { style: metadataStyle, source: "default" | "custom" };
return { style: DEFAULT_VERBATI_STYLE, source: "default" };
```
