...
```txt
None. Do not start PR59 until this control-doc alignment PR is merged and PR59 preflight is reviewed or explicitly approved.
```
...
```txt
BLOCKED_PENDING_PREFLIGHT — real read-only data may require OAuth or explicit boundary-only narrowing before any PR59 code.
```
...
PR59 is blocked pending preflight: real read-only data requires OAuth first unless maintainers explicitly narrow PR59 to boundary-only opaque refs with no real data access.
