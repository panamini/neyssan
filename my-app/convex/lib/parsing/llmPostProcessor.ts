...
function mapHeaderToField(headerRaw: string): string {
  ...
  // Stronger mapping for French headings (include common variants & accents)
  const introKeys = [
    "profile", "profil", "profile professionnel", "à propos", "à propos de moi", "a propos",
    "about", "summary", "résumé", "resume", "résumé professionnel", "profil professionnel",
    "objectif", "objective"
  ];
  if (introKeys.some(k => cleaned.includes(k))) return "introduction";

  const experienceKeys = [
    "experience", "expérience", "experiences", "expériences",
    "experience professionnelle", "expérience professionnelle", "parcours", "parcours professionnel",
    "work", "employment", "career", "professional experience", "missions"
  ];
  if (experienceKeys.some(k => cleaned.includes(k))) return "experience";

  const skillsKeys = [
    "skill", "skills", "competence", "competences", "compétence", "compétences", "atouts",
    "principales competences", "compétences principales", "technical", "proficien", "compétences techniques"
  ];
  if (skillsKeys.some(k => cleaned.includes(k))) return "skills";

  const educationKeys = [
    "education", "formation", "études", "studies", "formation académique", "diplome", "diplômé", "academic background"
  ];
  if (educationKeys.some(k => cleaned.includes(k))) return "education";

  const achievementsKeys = [
    "achievement", "achievements", "réalisation", "réalisations", "award", "awards",
    "certif", "certificat", "certifications", "publications"
  ];
  if (achievementsKeys.some(k => cleaned.includes(k))) return "achievements";

  const languagesKeys = ["language", "languages", "langue", "langues", "langues parlées"];
  if (languagesKeys.some(k => cleaned.includes(k))) return "languages";

  const contactKeys = [
    "contact", "coordonne", "coordonnées", "coordonnees", "adresse", "address",
    "email", "mail", "phone", "telephone", "téléphone", "linkedin", "links", "link"
  ];
  if (contactKeys.some(k => cleaned.includes(k))) return "contact";
  ...
  if (cleaned.includes("volunteer") || cleaned.includes("benevolat") || cleaned.includes("bénévolat") || cleaned.includes("volontariat")) return "volunteer";
  if (cleaned.includes("reference") || cleaned.includes("référence") || cleaned.includes("références")) return "references";
