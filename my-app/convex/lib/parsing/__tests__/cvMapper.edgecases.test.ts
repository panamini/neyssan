...
describe("mapSectionsToCV edge cases", () => {
  it("merges multi-language lists from different section formats", async () => {
    const sections = [
    ...
    const cv = await mapSectionsToCV(sections, {});
  ...
  it("aggregates fragmented contact info across multiple contact sections", async () => {
    const sections = [
    ...
    const cv = await mapSectionsToCV(sections, { name: "Foo Bar" });
  ...
  it("computes average confidence for low-confidence merged singular fields", async () => {
    const sections = [
  ...
  it("places unknown fieldKeys into other bucket", async () => {
    const sections = [
