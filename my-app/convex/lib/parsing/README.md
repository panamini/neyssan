...
Exports
- mapSectionsToCV(sections: IParsedSection[], metadata?: IParsedMetadata): Promise<ICVObject>
  - Main typed mapping function; throws on invalid input (validated with Zod).
- mapSectionsToCVSafe(input: unknown): Promise<ICVObject>
...
- Typical consumption after calling existing parse function:
  ...
  - const cv = await mapSectionsToCV(parsed.sections, parsed.metadata)
  - render CV using `cv.summary`, `cv.experience`, `cv.skills`, etc.
- The mapper is intentionally pure and relies only on the provided `sections` + `metadata`; it now calls the shared embedding client to rerank languages/skills and therefore returns a Promise.
