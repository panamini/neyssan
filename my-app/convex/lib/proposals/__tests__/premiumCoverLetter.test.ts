...
import {
  ...
  ENGLISH_NEUTRAL_EVIDENCE_CLOSE,
...
describe("premium ClaimPlan provenance v1", () => {
  it("allocates distinct structured English evidence even when facts share metrics", () => {
    const claimMetadata = {
      claimType: "source_backed" as const,
      requiredElements: [],
      allowedVerbs: [],
      forbiddenVerbs: [],
      forbiddenClaims: [],
      maxOwnership: "support" as const,
    ...
    const claimPlan = {
      ...
      claims: [
        ...
        },
        {
          ...
          section: "employerValueBlock" as const,
          factIds: ["fact_metric_a", "fact_metric_b"],
          demandIds: [],
          ...claimMetadata,
        },
        {
          id: "claim_close_001",
          section: "closeLine" as const,
          factIds: ["fact_metric_a", "fact_metric_b", "fact_distinct"],
          demandIds: [],
          ...claimMetadata,
        },
      ],
    ...
    const writerOutput = {
      ...
      bodyParts: {
        ...
        employerValueBlock: {
          section: "employerValueBlock" as const,
          text: "The work supports careful handoffs.",
        ...
        closeLine: {
          section: "closeLine" as const,
          text: "Close uses another fact.",
          claimIds: ["claim_close_001"],
          factIds: ["fact_metric_a", "fact_distinct"],
          demandIds: [],
        },
      },
    };

    const first = allocateEnglishCvBackedEvidence({ writerOutput, claimPlan });
    const second = allocateEnglishCvBackedEvidence({
      writerOutput: first.writerOutput,
      claimPlan,
    });

    expect(first.issues).toEqual([]);
    expect(first.neutralClose).toBe(false);
    expect(first.writerOutput.bodyParts.opening.factIds).toEqual([
      "fact_metric_a",
    ]);
    expect(first.writerOutput.bodyParts.proofBlock.factIds).toEqual([
      "fact_metric_b",
    ]);
    expect(first.writerOutput.bodyParts.closeLine.factIds).toEqual([
      "fact_distinct",
    ]);
    expect(second).toEqual(first);
  });

  it("fails closed when proof reallocates the same structured fact", () => {
    const { claimPlan } = buildDirectClaimPlanFixture();
    ...
    const writerOutput = buildDirectPremiumWriterOutputFixture({
      opening: "I used the opening evidence.",
      proofBlock: "I reused the opening evidence.",
      employerValueBlock: "The work supports careful handoffs.",
      closeLine: "I bring careful follow-through.",
    });
    const result = allocateEnglishCvBackedEvidence({
      writerOutput: {
        ...writerOutput,
        bodyParts: {
          ...
          proofBlock: {
            ...
            factIds: [openingClaim.factIds[0]!],
          },
        },
      },
      claimPlan: {
        ...
        claims: claimPlan.claims.map((claim) =>
          claim.id === proofClaim.id
            ? {
                ...claim,
                factIds: [openingClaim.factIds[0]!, "fact_distinct"],
              }
            : claim,
        ),
      },
    ...
    expect(result.issues).toEqual([
      {
        ...
        section: "proofBlock",
        factId: openingClaim.factIds[0],
      },
    ]);
  });

  it("selects fresh structured facts when a section mixes reused and fresh IDs", () => {
    const { claimPlan } = buildDirectClaimPlanFixture();
    const openingClaim = claimPlan.claims.find(
      (claim) => claim.section === "opening",
    )!;
    ...
    const freshFactId = "fact_distinct";
    const baseWriterOutput = buildDirectPremiumWriterOutputFixture({
      opening: "I used the opening evidence.",
      proofBlock: "I used fresh proof evidence.",
      employerValueBlock: "The work supports careful handoffs.",
      closeLine: "I would bring that evidence to the role.",
    });

    const proofResult = allocateEnglishCvBackedEvidence({
      writerOutput: {
        ...baseWriterOutput,
        bodyParts: {
          ...baseWriterOutput.bodyParts,
          proofBlock: {
            ...baseWriterOutput.bodyParts.proofBlock,
            factIds: [openingClaim.factIds[0]!, freshFactId],
          },
        },
      },
      claimPlan: {
        ...
        claims: claimPlan.claims.map((claim) =>
          claim.id === proofClaim.id
            ? { ...claim, factIds: [openingClaim.factIds[0]!, freshFactId] }
            : claim,
        ),
      },
    });

    expect(proofResult.issues).toEqual([]);
    expect(proofResult.writerOutput.bodyParts.proofBlock.factIds).toEqual([
      freshFactId,
    ]);
    expect(proofResult.writerOutput.bodyParts.proofBlock.claimIds).toEqual([
      proofClaim.id,
    ]);

    const closeResult = allocateEnglishCvBackedEvidence({
      writerOutput: {
        ...baseWriterOutput,
        bodyParts: {
          ...baseWriterOutput.bodyParts,
          closeLine: {
            ...baseWriterOutput.bodyParts.closeLine,
            claimIds: [proofClaim.id, closeClaim.id],
            factIds: [openingClaim.factIds[0]!, freshFactId],
          },
        },
      },
      claimPlan: {
        ...claimPlan,
        claims: claimPlan.claims.map((claim) =>
          claim.id === closeClaim.id
            ? {
                ...claim,
                factIds: [
                  openingClaim.factIds[0]!,
                  proofClaim.factIds[0]!,
                  freshFactId,
                ],
              }
            : claim,
    ...
    expect(closeResult.issues).toEqual([]);
    expect(closeResult.neutralClose).toBe(false);
    expect(closeResult.writerOutput.bodyParts.closeLine.factIds).toEqual([
      freshFactId,
    ]);
    expect(closeResult.writerOutput.bodyParts.closeLine.claimIds).toEqual([
      closeClaim.id,
    ]);
  });

  it("uses a neutral non-factual close when no distinct close fact exists", () => {
    const { claimPlan } = buildDirectClaimPlanFixture();
    const openingClaim = claimPlan.claims.find(
      (claim) => claim.section === "opening",
    )!;
    ...
    const baseWriterOutput = buildDirectPremiumWriterOutputFixture({
      opening: "I used the opening evidence.",
      proofBlock: "I used the proof evidence.",
      employerValueBlock: "The work supports careful handoffs.",
      closeLine: "I have no distinct close evidence.",
    });
    const writerOutput = {
      ...baseWriterOutput,
      bodyParts: {
    ...
    const allocationClaimPlan = {
      ...claimPlan,
      claims: claimPlan.claims.map((claim) =>
        claim.section === "closeLine"
          ? {
              ...claim,
              factIds: [openingClaim.factIds[0]!, proofClaim.factIds[0]!],
            }
          : claim,
      ),
    };
    const result = allocateEnglishCvBackedEvidence({
      writerOutput,
      claimPlan: allocationClaimPlan,
    });

    expect(result.issues).toEqual([]);
    expect(result.neutralClose).toBe(true);
    expect(result.writerOutput.bodyParts.closeLine.text).toBe(
      ENGLISH_NEUTRAL_EVIDENCE_CLOSE,
    );
    expect(result.writerOutput.bodyParts.closeLine.factIds).toEqual([]);
    expect(
      allocateEnglishCvBackedEvidence({
  ...
  it("neutralizes a close that reallocates only already-used structured facts", () => {
    const { claimPlan } = buildDirectClaimPlanFixture();
    const openingClaim = claimPlan.claims.find(
      (claim) => claim.section === "opening",
    )!;
    ...
    const baseWriterOutput = buildDirectPremiumWriterOutputFixture({
      ...
      proofBlock: "I used the proof evidence.",
      employerValueBlock: "The work supports careful handoffs.",
      closeLine: "That same evidence would support the role.",
    });
    const allocationClaimPlan = {
      ...claimPlan,
      claims: claimPlan.claims.map((claim) =>
  ...
  it("locks evidence allocation to structured English CV-backed non-Mistral output", () => {
    ...
    for (const outputLanguage of otherLanguages) {
      expect(
        isEnglishCvBackedEvidenceAllocationActive({
          outputLanguage,
          contextClass: "cv_direct",
