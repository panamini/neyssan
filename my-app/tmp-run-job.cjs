...
var llmPostProcessor_exports = {};
__export(llmPostProcessor_exports, {
  parseLLMMetadata: () => parseLLMMetadata,
  parseLLMSections: () => parseLLMSections,
  recoverSpans: () => recoverSpans
});
function safeJsonParse(s2) {
  try {
...
function extractJsonFence(text) {
  ...
  if (braceIndex >= 0) {
    let depth = 0;
    for (let i2 = braceIndex; i2 < text.length; i2++) {
      const ch = text[i2];
      if (ch === "{")
        depth++;
      else if (ch === "}") {
        depth--;
        if (depth === 0) {
          return text.slice(braceIndex, i2 + 1);
        }
      }
    }
  }
  try {
    ...
    if (jsonFenceUnesc && jsonFenceUnesc[1])
      return jsonFenceUnesc[1].trim();
    const braceIndexUnesc = unescaped.indexOf("{");
    if (braceIndexUnesc >= 0) {
      let depth = 0;
      for (let i2 = braceIndexUnesc; i2 < unescaped.length; i2++) {
        const ch = unescaped[i2];
        if (ch === "{")
          depth++;
        else if (ch === "}") {
          depth--;
          if (depth === 0) {
            return unescaped.slice(braceIndexUnesc, i2 + 1);
          }
        }
      }
    }
...
function deaccent2(s2) {
  try {
    return s2.normalize("NFD").replace(/\p{Diacritic}/gu, "");
  } catch {
    return s2;
  }
}
...
function mapHeaderToField(headerRaw) {
  const cleaned = deaccent2(String(headerRaw || "")).replace(/[\u2018\u2019\u201C\u201D]/g, "").replace(/[^a-zA-Z0-9\s]/g, " ").toLowerCase().replace(/\s+/g, " ").trim();
  if (!cleaned)
    return "introduction";
  const introKeys = [
    "profile",
    ...
    "r\xE9sum\xE9",
    "resume",
    "r\xE9sum\xE9 professionnel",
    "profil professionnel",
    "objectif",
    "objective"
  ];
  if (introKeys.some((k2) => cleaned.includes(k2)))
    return "introduction";
  ...
  if (experienceKeys.some((k2) => cleaned.includes(k2)))
    return "experience";
  const skillsKeys = [
    "skill",
    "skills",
    ...
    "comp\xE9tences principales",
    "technical",
    "proficien",
    "comp\xE9tences techniques"
  ];
  if (skillsKeys.some((k2) => cleaned.includes(k2)))
    return "skills";
  const educationKeys = [
    "education",
    "formation",
    "\xE9tudes",
    "studies",
    "formation acad\xE9mique",
    "diplome",
    "dipl\xF4m\xE9",
    "academic background"
  ];
  if (educationKeys.some((k2) => cleaned.includes(k2)))
    return "education";
  ...
  if (achievementsKeys.some((k2) => cleaned.includes(k2)))
    return "achievements";
  const languagesKeys = ["language", "languages", "langue", "langues", "langues parl\xE9es"];
  if (languagesKeys.some((k2) => cleaned.includes(k2)))
    return "languages";
  const contactKeys = [
    "contact",
    "coordonne",
  ...
  if (contactKeys.some((k2) => cleaned.includes(k2)))
    return "contact";
  if (cleaned.includes("project") || cleaned.includes("projet") || cleaned.includes("projets"))
    return "projects";
  if (cleaned.includes("research") || cleaned.includes("recherche"))
    return "research";
  if (cleaned.includes("volunteer") || cleaned.includes("benevolat") || cleaned.includes("b\xE9n\xE9volat") || cleaned.includes("volontariat"))
    return "volunteer";
  if (cleaned.includes("reference") || cleaned.includes("r\xE9f\xE9rence") || cleaned.includes("r\xE9f\xE9rences"))
    return "references";
  if (cleaned.includes("autre") || cleaned.includes("divers") || cleaned.includes("other"))
    return "other";
  return "introduction";
}
function inferFieldFromContent(title, content, initial) {
  const lc = String(content || "").toLowerCase();
  const t2 = String(title || "").toLowerCase();
  if (/\b[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}\b/i.test(content) || /\+?\d{2,3}[\s.\-]?\d{1,4}/.test(content) || t2.includes("coordonn"))
    return "contact";
  if (/\b(19|20)\d{2}\b/.test(content) || /\bprésent\b/i.test(lc) || /\bpresent\b/i.test(lc) || /\b(ans|année|years)\b/i.test(lc) || /\b(agent|directeur|ingénieur|responsable|manager|chef|lead|consultant)\b/i.test(lc))
    return "experience";
  if (/\b(universit|école|school|licence|master|mba|diplôm|bachelor)\b/i.test(content) || t2.includes("formation"))
    return "education";
  if (t2.includes("comp\xE9t") || t2.includes("skill") || content.split(/\r?\n/).length <= 8 && content.split(/[,;\n•·\u2022-]+/).length >= 2 && content.length < 500)
    return "skills";
  if (t2.includes("lang") || /\b(francais|français|anglais|english|espagnol|italien|german|allemand)\b/i.test(lc))
    return "languages";
  if (t2.includes("projet") || t2.includes("project"))
    return "projects";
  if (t2.includes("recherche") || t2.includes("research"))
    return "research";
  return initial;
}
function splitByMarkdownHeaders(text) {
  ...
  while ((m2 = regex.exec(text)) !== null) {
    matches.push({ index: m2.index, header: m2[2].trim() });
  }
  if (matches.length) {
    const sections = [];
    for (let i2 = 0; i2 < matches.length; i2++) {
      const start = matches[i2].index;
      const end = i2 + 1 < matches.length ? matches[i2 + 1].index : text.length;
      const headerLine = text.slice(start, end).split(/\r?\n/)[0] || matches[i2].header;
      const headerText = headerLine.replace(/^\s*#{1,4}\s*/g, "").trim() || matches[i2].header;
...
function splitByBoldHeadings(text) {
  ...
  for (let i2 = 0; i2 < lines.length; i2++) {
    ...
    if (boldMatch) {
      ...
      candidates.push({ index: i2, header });
      continue;
    }
    const deLine = deaccent2(line);
    if (!deLine)
      continue;
    ...
    const containsHint = headerKeywordHints.some((h2) => lower.includes(h2));
    const acceptAsHeader = (isAllCapsLike || isTitleCase) && (wordCount <= 5 || containsHint);
    if (line && acceptAsHeader) {
      candidates.push({ index: i2, header: rawLine.trim() });
    }
  }
  if (candidates.length) {
    const sections = [];
    for (let i2 = 0; i2 < candidates.length; i2++) {
      const startLine = candidates[i2].index;
      const endLine = i2 + 1 < candidates.length ? candidates[i2 + 1].index : lines.length;
      const bodyLines = lines.slice(startLine + 1, endLine);
      sections.push({ title: candidates[i2].header, content: bodyLines.join("\n").trim() });
    }
    return sections;
  }
  return null;
...
function parseLLMSections(text) {
  function extractFromProviderWrapper(raw) {
    if (!raw)
      return raw;
    try {
      ...
      if (parsed && typeof parsed === "object") {
        ...
        if (parsed.full_response && Array.isArray(parsed.full_response.choices) && parsed.full_response.choices[0]?.message?.content)
          return String(parsed.full_response.choices[0].message.content);
        if (parsed.output && Array.isArray(parsed.output)) {
          for (const item of parsed.output) {
            ...
            if (Array.isArray(item.content) && item.content.length) {
              for (const c2 of item.content) {
                if (typeof c2 === "string")
                  return c2;
                if (c2 && typeof c2 === "object" && typeof c2.text === "string")
                  return c2.text;
              }
            }
          }
        }
        if (Array.isArray(parsed.sections) || parsed.profile || parsed.experience || parsed.skills || parsed.contact)
  ...
  if (sanitizedOnce && sanitizedOnce !== text)
    text = sanitizedOnce;
  const direct = safeJsonParse(text);
  if (direct && Array.isArray(direct.sections)) {
    const secs = direct.sections.map((s2) => ({
      title: String(s2.title || "Section"),
      content: String(s2.content || ""),
      fieldKey: String(s2.fieldKey || mapHeaderToField(s2.title || "")),
      confidence: typeof s2.confidence === "number" ? Math.max(0, Math.min(1, s2.confidence)) : 0.85
    }));
    return { sections: secs };
  }
  const jsonFence = extractJsonFence(text);
  if (jsonFence) {
    const parsedFence = safeJsonParse(jsonFence);
    if (parsedFence && Array.isArray(parsedFence.sections)) {
      const secs = parsedFence.sections.map((s2) => ({
        title: String(s2.title || "Section"),
        content: String(s2.content || ""),
        fieldKey: String(s2.fieldKey || mapHeaderToField(s2.title || "")),
        confidence: typeof s2.confidence === "number" ? Math.max(0, Math.min(1, s2.confidence)) : 0.85
    ...
    if (parsedFence && typeof parsedFence === "object") {
      ...
      if (parsedFence.profile || parsedFence.summary || parsedFence.introduction) {
        const p2 = String(parsedFence.profile ?? parsedFence.summary ?? parsedFence.introduction ?? "");
        if (p2.trim())
          altSections.push({ title: "Introduction", content: p2.trim() });
      }
      if (Array.isArray(parsedFence.experience) && parsedFence.experience.length) {
        const body = parsedFence.experience.map((e2) => {
          const lines = [];
          if (e2.position || e2.title)
            lines.push(String(e2.position ?? e2.title));
          if (e2.company)
            lines.push(String(e2.company));
          if (e2.duration || e2.start || e2.end)
            lines.push(String(e2.duration ?? `${e2.start ?? ""} - ${e2.end ?? ""}`).trim());
          if (e2.responsibilities && Array.isArray(e2.responsibilities))
            lines.push(e2.responsibilities.join("\n"));
          return lines.filter(Boolean).join("\n");
        }).join("\n\n");
        if (body.trim())
          altSections.push({ title: "Experience", content: body.trim() });
      }
      if (Array.isArray(parsedFence.skills) && parsedFence.skills.length) {
        const skills = parsedFence.skills.map(String).join(", ");
      ...
      if (Array.isArray(parsedFence.education) && parsedFence.education.length) {
        const eduBody = parsedFence.education.map((e2) => {
          const school = e2.school ?? e2.institution ?? "";
          const degree = e2.degree ?? e2.program ?? "";
          return [school, degree].filter(Boolean).join(" - ");
        }).join("\n");
        if (eduBody.trim())
          altSections.push({ title: "Education", content: eduBody.trim() });
      }
      if (parsedFence.contact || parsedFence.contact_info || parsedFence.contactInfo) {
        const c2 = parsedFence.contact ?? parsedFence.contact_info ?? parsedFence.contactInfo;
        let contactText = "";
        if (typeof c2 === "string")
          contactText = c2;
          ...
          const parts = [];
          if (c2.email)
            parts.push(String(c2.email));
          if (c2.phone)
            parts.push(String(c2.phone));
          if (c2.linkedin)
            parts.push(String(c2.linkedin));
          contactText = parts.join("\n");
        }
        if (contactText.trim())
          altSections.push({ title: "Contact", content: contactText.trim() });
      }
      if (altSections.length === 0) {
        const candidateKeys = ["profile", "summary", "experience", "skills", "education", "contact"];
        for (const k2 of Object.keys(parsedFence)) {
          if (candidateKeys.includes(k2))
            continue;
          const val = parsedFence[k2];
          if (typeof val === "string" && val.trim().length > 0 && val.trim().length < 2e3) {
            altSections.push({ title: String(k2), content: String(val).trim() });
          }
        }
      }
      if (altSections.length) {
        return {
          sections: altSections.map((s2) => ({
            title: s2.title,
            content: s2.content || "",
            fieldKey: mapHeaderToField(s2.title),
            confidence: 0.85
          }))
        };
      }
  ...
  if (bold) {
    const post = bold.map((s2) => {
      const initial = mapHeaderToField(s2.title);
      const inferred = inferFieldFromContent(s2.title, s2.content || "", initial);
      return {
        title: s2.title,
        content: s2.content || "",
        fieldKey: inferred,
  ...
  function simpleHeaderScan(fullText) {
    const lines = fullText.split(/\r?\n/).map((l2) => l2.replace(/\t/g, " ").trim());
    ...
    const sections = [];
    let current = null;
    const isProbablyHeader = (line) => {
      if (!line)
        return false;
      const normalized = deaccent2(line).toLowerCase();
      if (normalized.length > 140)
        return false;
      if (line.trim().endsWith(":"))
        return true;
      for (const cand of headerCandidates) {
        const normalizedCand = deaccent2(cand).toLowerCase();
        if (normalized === normalizedCand || normalized.includes(normalizedCand))
          return true;
      }
      if (/^[\p{Lu}\s0-9_-]{2,80}$/u.test(deaccent2(line)) && line.split(/\s+/).length <= 3)
        return true;
      return false;
    };
    for (let i2 = 0; i2 < lines.length; i2++) {
    ...
    if (current)
      sections.push(current);
    return sections.map((s2) => ({ title: s2.title || "Section", content: s2.content.join("\n").trim() })).filter((s2) => s2.title || s2.content);
  }
  const heur = simpleHeaderScan(text);
  if (heur && heur.length > 0) {
    const post = heur.map((s2) => {
      const initial = mapHeaderToField(s2.title);
      const inferred = inferFieldFromContent(s2.title, s2.content, initial);
      return {
        title: s2.title,
...
function normalizeForMatch(s2) {
  return deaccent2(
    String(s2 || "").replace(/```[\s\S]*?```/g, " ").replace(/https?:\/\/[^\s)]+/g, " ").replace(/<[^>]+>/g, " ").replace(/[_*~`>#-]{1,}/g, " ").replace(/[\u2018\u2019\u201C\u201D]/g, "").replace(/[^\w\s]/g, " ").replace(/\s+/g, " ").trim().toLowerCase()
  );
}
function findSnippetIndexInNormalized(snippet, originalNormalized) {
  if (!snippet)
    return -1;
  return originalNormalized.indexOf(snippet);
}
function mapNormalizedIndexToRawSpan(originalRaw, originalNormalized, matchIndex, normalizedMatchLen) {
  if (matchIndex < 0)
    return null;
  ...
  const m2 = regex.exec(originalRaw);
...
function recoverSpans(parsed, originalText) {
  ...
  return parsed.map((section) => {
    ...
    let foundSpan = null;
    for (let n2 = maxSnippetWords; n2 >= 4 && !foundSpan; n2--) {
      const snippet = words.slice(0, n2).join(" ");
    ...
    if (!foundSpan && words.length >= 4) {
      for (let i2 = 0; i2 <= words.length - 4 && !foundSpan; i2++) {
        const snippet = words.slice(i2, i2 + 4).join(" ");
        const matchIdx = findSnippetIndexInNormalized(snippet, originalNormalized);
        if (matchIdx !== -1) {
          const span = mapNormalizedIndexToRawSpan(originalText, originalNormalized, matchIdx, snippet.length);
          if (span)
...
function findEmail(text) {
  const m2 = text.match(/[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.[a-z]{2,}/i);
  return m2 ? m2[0] : null;
}
function findPhone(text) {
  const m2 = text.match(/(\+?\d{1,3}[-.\s]?)?(\(?\d{2,4}\)?[-.\s]?)?[\d\-\s]{5,16}/);
  if (!m2)
    return null;
  const candidate = m2[0].replace(/\s{2,}/g, " ").trim();
  if (candidate.match(/\b(19|20)\d{2}\b/))
    return null;
  return candidate;
}
function findLinkedin(text) {
  const m2 = text.match(/https?:\/\/(www\.)?linkedin\.com\/[^\s)]+/i);
  return m2 ? m2[0] : null;
}
function findNameFromTop(text) {
...
function parseLLMMetadata(text) {
  const direct = safeJsonParse(text);
  if (direct && (direct.name !== void 0 || direct.email !== void 0 || direct.phone !== void 0)) {
    return {
...
var init_llmPostProcessor = __esm({
  "my-app/convex/lib/parsing/llmPostProcessor.ts"() {
    "use strict";
    try {
      if (typeof global !== "undefined") {
        global.parseLLMSections = parseLLMSections;
        global.parseLLMMetadata = parseLLMMetadata;
        global.recoverSpans = recoverSpans;
      }
    } catch {
    }
  }
...
// my-app/node_modules/openai/internal/qs/formats.mjs
...
// my-app/node_modules/openai/internal/qs/utils.mjs
...
// my-app/node_modules/openai/internal/qs/stringify.mjs
...
// my-app/node_modules/openai/internal/qs/index.mjs
var init_qs = __esm({
  "my-app/node_modules/openai/internal/qs/index.mjs"() {
    init_stringify();
  }
...
// my-app/node_modules/openai/version.mjs
var VERSION;
...
// my-app/node_modules/openai/_shims/registry.mjs
...
// my-app/node_modules/openai/node_modules/webidl-conversions/lib/index.js
var require_lib = __commonJS({
  "my-app/node_modules/openai/node_modules/webidl-conversions/lib/index.js"(exports, module2) {
    "use strict";
    var conversions = {};
...
// my-app/node_modules/openai/node_modules/whatwg-url/lib/utils.js
var require_utils = __commonJS({
  "my-app/node_modules/openai/node_modules/whatwg-url/lib/utils.js"(exports, module2) {
    "use strict";
...
// my-app/node_modules/openai/node_modules/tr46/lib/mappingTable.json
var require_mappingTable = __commonJS({
  "my-app/node_modules/openai/node_modules/tr46/lib/mappingTable.json"(exports, module2) {
...
// my-app/node_modules/openai/node_modules/tr46/index.js
var require_tr46 = __commonJS({
  "my-app/node_modules/openai/node_modules/tr46/index.js"(exports, module2) {
    "use strict";
...
// my-app/node_modules/openai/node_modules/whatwg-url/lib/url-state-machine.js
var require_url_state_machine = __commonJS({
  "my-app/node_modules/openai/node_modules/whatwg-url/lib/url-state-machine.js"(exports, module2) {
    "use strict";
    var punycode = require("punycode");
...
// my-app/node_modules/openai/node_modules/whatwg-url/lib/URL-impl.js
var require_URL_impl = __commonJS({
  "my-app/node_modules/openai/node_modules/whatwg-url/lib/URL-impl.js"(exports) {
    "use strict";
...
// my-app/node_modules/openai/node_modules/whatwg-url/lib/URL.js
var require_URL = __commonJS({
  "my-app/node_modules/openai/node_modules/whatwg-url/lib/URL.js"(exports, module2) {
    "use strict";
...
// my-app/node_modules/openai/node_modules/whatwg-url/lib/public-api.js
var require_public_api = __commonJS({
  "my-app/node_modules/openai/node_modules/whatwg-url/lib/public-api.js"(exports) {
    "use strict";
...
// my-app/node_modules/openai/node_modules/node-fetch/lib/index.js
var require_lib2 = __commonJS({
  "my-app/node_modules/openai/node_modules/node-fetch/lib/index.js"(exports, module2) {
    "use strict";
...
// my-app/node_modules/formdata-node/node_modules/web-streams-polyfill/dist/ponyfill.mjs
function t() {
}
function r(e2) {
...
// my-app/node_modules/formdata-node/lib/esm/isFunction.js
var isFunction;
...
// my-app/node_modules/formdata-node/lib/esm/blobHelpers.js
...
// my-app/node_modules/formdata-node/lib/esm/Blob.js
...
// my-app/node_modules/formdata-node/lib/esm/File.js
...
// my-app/node_modules/formdata-node/lib/esm/isFile.js
var isFile;
...
// my-app/node_modules/formdata-node/lib/esm/isBlob.js
var isBlob;
...
// my-app/node_modules/formdata-node/lib/esm/deprecateConstructorEntries.js
var import_util, deprecateConstructorEntries;
var init_deprecateConstructorEntries = __esm({
  "my-app/node_modules/formdata-node/lib/esm/deprecateConstructorEntries.js"() {
    import_util = require("util");
    deprecateConstructorEntries = (0, import_util.deprecate)(() => {
    }, 'Constructor "entries" argument is not spec-compliant and will be removed in next major release.');
  }
});

// my-app/node_modules/formdata-node/lib/esm/FormData.js
var import_util2, __classPrivateFieldGet3, _FormData_instances, _FormData_entries, _FormData_setEntry, FormData2;
...
// my-app/node_modules/formdata-node/lib/esm/index.js
var init_esm = __esm({
  "my-app/node_modules/formdata-node/lib/esm/index.js"() {
    init_FormData();
    init_Blob();
...
// my-app/node_modules/ms/index.js
var require_ms = __commonJS({
  "my-app/node_modules/ms/index.js"(exports, module2) {
    var s2 = 1e3;
    var m2 = s2 * 60;
    var h2 = m2 * 60;
    var d2 = h2 * 24;
...
// my-app/node_modules/humanize-ms/index.js
var require_humanize_ms = __commonJS({
  "my-app/node_modules/humanize-ms/index.js"(exports, module2) {
    "use strict";
...
// my-app/node_modules/agentkeepalive/lib/constants.js
var require_constants = __commonJS({
  "my-app/node_modules/agentkeepalive/lib/constants.js"(exports, module2) {
    "use strict";
    module2.exports = {
      // agent
...
// my-app/node_modules/agentkeepalive/lib/agent.js
var require_agent = __commonJS({
  "my-app/node_modules/agentkeepalive/lib/agent.js"(exports, module2) {
    "use strict";
...
// my-app/node_modules/agentkeepalive/lib/https_agent.js
var require_https_agent = __commonJS({
  "my-app/node_modules/agentkeepalive/lib/https_agent.js"(exports, module2) {
    "use strict";
...
// my-app/node_modules/agentkeepalive/index.js
var require_agentkeepalive = __commonJS({
  "my-app/node_modules/agentkeepalive/index.js"(exports, module2) {
    "use strict";
    var HttpAgent = require_agent();
...
// my-app/node_modules/event-target-shim/dist/event-target-shim.js
var require_event_target_shim = __commonJS({
  "my-app/node_modules/event-target-shim/dist/event-target-shim.js"(exports, module2) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
...
// my-app/node_modules/abort-controller/dist/abort-controller.js
var require_abort_controller = __commonJS({
  "my-app/node_modules/abort-controller/dist/abort-controller.js"(exports, module2) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
...
// my-app/node_modules/form-data-encoder/lib/esm/util/createBoundary.js
...
// my-app/node_modules/form-data-encoder/lib/esm/util/isPlainObject.js
...
// my-app/node_modules/form-data-encoder/lib/esm/util/normalizeValue.js
...
// my-app/node_modules/form-data-encoder/lib/esm/util/escapeName.js
...
// my-app/node_modules/form-data-encoder/lib/esm/util/isFunction.js
...
// my-app/node_modules/form-data-encoder/lib/esm/util/isFileLike.js
var isFileLike;
...
// my-app/node_modules/form-data-encoder/lib/esm/util/isFormData.js
var isFormData;
...
// my-app/node_modules/form-data-encoder/lib/esm/FormDataEncoder.js
...
// my-app/node_modules/form-data-encoder/lib/esm/FileLike.js
var init_FileLike = __esm({
  "my-app/node_modules/form-data-encoder/lib/esm/FileLike.js"() {
  }
});

// my-app/node_modules/form-data-encoder/lib/esm/FormDataLike.js
var init_FormDataLike = __esm({
  "my-app/node_modules/form-data-encoder/lib/esm/FormDataLike.js"() {
  }
});

// my-app/node_modules/form-data-encoder/lib/esm/index.js
var init_esm2 = __esm({
  "my-app/node_modules/form-data-encoder/lib/esm/index.js"() {
...
// my-app/node_modules/openai/_shims/MultipartBody.mjs
var MultipartBody;
...
// my-app/node_modules/node-domexception/index.js
var require_node_domexception = __commonJS({
  "my-app/node_modules/node-domexception/index.js"(exports, module2) {
...
// my-app/node_modules/formdata-node/lib/esm/isPlainObject.js
...
// my-app/node_modules/formdata-node/lib/esm/fileFromPath.js
...
// my-app/node_modules/openai/_shims/node-runtime.mjs
...
// my-app/node_modules/openai/_shims/auto/runtime-node.mjs
...
// my-app/node_modules/openai/_shims/index.mjs
var init;
...
// my-app/node_modules/openai/error.mjs
...
var init_error = __esm({
  "my-app/node_modules/openai/error.mjs"() {
    ...
    APIError = class extends OpenAIError {
...
// my-app/node_modules/openai/internal/decoders/line.mjs
...
// my-app/node_modules/openai/internal/stream-utils.mjs
...
// my-app/node_modules/openai/streaming.mjs
...
// my-app/node_modules/openai/uploads.mjs
...
// my-app/node_modules/openai/core.mjs
...
// my-app/node_modules/openai/pagination.mjs
var Page, CursorPage;
...
// my-app/node_modules/openai/resource.mjs
var APIResource;
...
// my-app/node_modules/openai/resources/chat/completions/messages.mjs
var Messages;
...
// my-app/node_modules/openai/resources/chat/completions/completions.mjs
...
// my-app/node_modules/openai/resources/chat/chat.mjs
var Chat;
var init_chat = __esm({
  "my-app/node_modules/openai/resources/chat/chat.mjs"() {
    init_resource();
...
// my-app/node_modules/openai/resources/chat/index.mjs
...
// my-app/node_modules/openai/resources/shared.mjs
var init_shared = __esm({
  "my-app/node_modules/openai/resources/shared.mjs"() {
  }
});

// my-app/node_modules/openai/resources/audio/speech.mjs
var Speech;
...
// my-app/node_modules/openai/resources/audio/transcriptions.mjs
var Transcriptions;
...
// my-app/node_modules/openai/resources/audio/translations.mjs
var Translations;
...
// my-app/node_modules/openai/resources/audio/audio.mjs
var Audio;
...
// my-app/node_modules/openai/resources/batches.mjs
...
// my-app/node_modules/openai/lib/EventStream.mjs
...
// my-app/node_modules/openai/lib/AssistantStream.mjs
...
// my-app/node_modules/openai/resources/beta/assistants.mjs
...
// my-app/node_modules/openai/lib/RunnableFunction.mjs
...
// my-app/node_modules/openai/lib/chatCompletionUtils.mjs
...
// my-app/node_modules/openai/lib/parser.mjs
...
// my-app/node_modules/openai/lib/AbstractChatCompletionRunner.mjs
...
// my-app/node_modules/openai/lib/ChatCompletionRunner.mjs
var ChatCompletionRunner;
var init_ChatCompletionRunner = __esm({
  "my-app/node_modules/openai/lib/ChatCompletionRunner.mjs"() {
    init_AbstractChatCompletionRunner();
    init_chatCompletionUtils();
    ChatCompletionRunner = class extends AbstractChatCompletionRunner {
...
// my-app/node_modules/openai/_vendor/partial-json-parser/parser.mjs
...
// my-app/node_modules/openai/lib/ChatCompletionStream.mjs
...
var init_ChatCompletionStream = __esm({
  "my-app/node_modules/openai/lib/ChatCompletionStream.mjs"() {
    ...
    ChatCompletionStream = class extends AbstractChatCompletionRunner {
      constructor(params) {
        super();
...
// my-app/node_modules/openai/lib/ChatCompletionStreamingRunner.mjs
var ChatCompletionStreamingRunner;
var init_ChatCompletionStreamingRunner = __esm({
  "my-app/node_modules/openai/lib/ChatCompletionStreamingRunner.mjs"() {
    init_ChatCompletionStream();
    ChatCompletionStreamingRunner = class extends ChatCompletionStream {
      static fromReadableStream(stream) {
...
// my-app/node_modules/openai/resources/beta/chat/completions.mjs
var Completions2;
...
// my-app/node_modules/openai/resources/beta/chat/chat.mjs
var Chat2;
...
// my-app/node_modules/openai/resources/beta/realtime/sessions.mjs
var Sessions;
...
// my-app/node_modules/openai/resources/beta/realtime/transcription-sessions.mjs
...
// my-app/node_modules/openai/resources/beta/realtime/realtime.mjs
var Realtime;
...
// my-app/node_modules/openai/resources/beta/threads/messages.mjs
...
// my-app/node_modules/openai/resources/beta/threads/runs/steps.mjs
...
// my-app/node_modules/openai/resources/beta/threads/runs/runs.mjs
var Runs, RunsPage;
var init_runs = __esm({
  "my-app/node_modules/openai/resources/beta/threads/runs/runs.mjs"() {
    init_resource();
...
// my-app/node_modules/openai/resources/beta/threads/threads.mjs
var Threads;
...
// my-app/node_modules/openai/resources/beta/beta.mjs
var Beta;
var init_beta = __esm({
  "my-app/node_modules/openai/resources/beta/beta.mjs"() {
    init_resource();
...
// my-app/node_modules/openai/resources/completions.mjs
var Completions3;
...
// my-app/node_modules/openai/resources/containers/files/content.mjs
var Content;
...
// my-app/node_modules/openai/resources/containers/files/files.mjs
...
// my-app/node_modules/openai/resources/containers/containers.mjs
...
// my-app/node_modules/openai/resources/embeddings.mjs
var Embeddings;
...
// my-app/node_modules/openai/resources/evals/runs/output-items.mjs
...
// my-app/node_modules/openai/resources/evals/runs/runs.mjs
...
// my-app/node_modules/openai/resources/evals/evals.mjs
...
// my-app/node_modules/openai/resources/files.mjs
...
// my-app/node_modules/openai/resources/fine-tuning/methods.mjs
var Methods;
...
// my-app/node_modules/openai/resources/fine-tuning/alpha/graders.mjs
var Graders;
...
// my-app/node_modules/openai/resources/fine-tuning/alpha/alpha.mjs
var Alpha;
...
// my-app/node_modules/openai/resources/fine-tuning/checkpoints/permissions.mjs
...
// my-app/node_modules/openai/resources/fine-tuning/checkpoints/checkpoints.mjs
var Checkpoints;
...
// my-app/node_modules/openai/resources/fine-tuning/jobs/checkpoints.mjs
...
// my-app/node_modules/openai/resources/fine-tuning/jobs/jobs.mjs
...
// my-app/node_modules/openai/resources/fine-tuning/fine-tuning.mjs
var FineTuning;
...
// my-app/node_modules/openai/resources/graders/grader-models.mjs
var GraderModels;
...
// my-app/node_modules/openai/resources/graders/graders.mjs
var Graders2;
...
// my-app/node_modules/openai/resources/images.mjs
var Images;
...
// my-app/node_modules/openai/resources/models.mjs
...
// my-app/node_modules/openai/resources/moderations.mjs
var Moderations;
...
// my-app/node_modules/openai/lib/ResponsesParser.mjs
...
// my-app/node_modules/openai/resources/responses/input-items.mjs
var InputItems;
...
// my-app/node_modules/openai/lib/responses/ResponseStream.mjs
...
var init_ResponseStream = __esm({
  "my-app/node_modules/openai/lib/responses/ResponseStream.mjs"() {
    ...
    ResponseStream = class extends EventStream {
      constructor(params) {
        super();
...
// my-app/node_modules/openai/resources/responses/responses.mjs
...
// my-app/node_modules/openai/resources/uploads/parts.mjs
var Parts;
...
// my-app/node_modules/openai/resources/uploads/uploads.mjs
var Uploads;
...
// my-app/node_modules/openai/lib/Util.mjs
...
// my-app/node_modules/openai/resources/vector-stores/files.mjs
...
// my-app/node_modules/openai/resources/vector-stores/file-batches.mjs
var FileBatches;
...
// my-app/node_modules/openai/resources/vector-stores/vector-stores.mjs
...
// my-app/node_modules/openai/resources/index.mjs
...
// my-app/node_modules/openai/index.mjs
var openai_exports = {};
...
function isPotentialHeader(line, context) {
  const trimmed = line.trim();
  const { previousLine, nextLine, lineIndex } = context;
  if (trimmed.length > 80 || trimmed.length < 2)
    return false;
  const isKnownHeader = Object.values(FIELD_KEY_MAP).some(
    (patterns) => patterns.some((pattern) => {
  ...
  let totalScore = 0;
  if (isAllCaps)
    totalScore += scores.isAllCaps;
  if (hasHeaderFormatting)
    totalScore += scores.hasHeaderFormatting;
  if (precededByEmptyLine)
    totalScore += scores.precededByEmptyLine;
  if (followedBySeparator)
    totalScore += scores.followedBySeparator;
  if (hasHighCapitalRatio)
    totalScore += scores.hasHighCapitalRatio;
  return totalScore >= 5;
}

// my-app/convex/lib/parsing/metadataExtractor.ts
function extractMetadataHeuristically(text) {
  ...
  const strategies = [
    ...
    () => {
      const candidate = lines.find(
        (line) => line.length > 3 && line.length < 40 && /^[A-Z][a-z]+(\s+[A-Z][a-z]+)+$/.test(line) && !line.includes("@") && !line.match(/\d/)
      );
    ...
    () => {
      for (let i2 = 0; i2 < Math.min(5, lines.length); i2++) {
        const words = lines[i2].split(/\s+/);
        if (words.length >= 2 && words.length <= 4) {
          const allCapitalized = words.every(
            (word) => word.length > 1 && /^[A-Z][a-z]*$/.test(word)
          );
          if (allCapitalized)
            return lines[i2];
        }
...
function extractPhone(text) {
  ...
  scoredPhones.sort((a2, b2) => b2.score - a2.score);
...
// my-app/convex/lib/parsing/llmValidator.ts
function deaccent(s2) {
  try {
    return s2.normalize("NFD").replace(/\p{Diacritic}/gu, "");
  } catch {
    return s2;
...
function stripMarkdownAndPunctuation(s2) {
  if (!s2)
    return "";
  let out = String(s2).replace(/```[\s\S]*?```/g, " ").replace(/https?:\/\/[^\s)]+/g, " ").replace(/<[^>]+>/g, " ").replace(/[_*~`>#-]{1,}/g, " ").replace(/[\u2018\u2019\u201C\u201D]/g, "").replace(/[^\w\s]/g, " ").replace(/\s+/g, " ").trim().toLowerCase();
  out = deaccent(out);
  return out;
}
function validateLLMOutput(output, originalText) {
  const issues = [];
  ...
  const confidences = output.sections.map((s2) => typeof s2.confidence === "number" ? s2.confidence : 0.5);
  const avgConfidence = confidences.reduce((a2, b2) => a2 + b2, 0) / confidences.length;
  ...
  matchedRanges.sort((a2, b2) => a2[0] - b2[0]);
  let covered = 0;
  let curStart = -1;
  let curEnd = -1;
  for (const [s2, e2] of matchedRanges) {
    if (s2 > curEnd) {
      if (curEnd > curStart)
  ...
  const sectionTitles = output.sections.map((s2) => stripMarkdownAndPunctuation(s2.title || ""));
  const duplicateTitles = sectionTitles.filter(
    (title, i2) => title && sectionTitles.indexOf(title) !== i2
  );
  if (duplicateTitles.length > 0) {
    issues.push(`Duplicate section titles: ${[...new Set(duplicateTitles)].join(", ")}`);
  }
  const isValid = issues.length === 0 && avgConfidence > 0.45 && minConfidence > 0.25 && coverageRatio > 0.25;
  ...
  if (!isValid) {
    try {
      console.debug("[llmValidator] validation failed", {
        ...
        coverageRatio,
        issues,
        sections: output.sections.map((s2, i2) => ({
          index: i2,
          title: s2.title,
          snippet: stripMarkdownAndPunctuation(String(s2.content || "")).split(/\s+/).slice(0, 12).join(" ")
        }))
      });
    } catch {
    }
...
// my-app/convex/lib/parsing/llmPrompts.ts
var SECTION_EXTRACTION_PROMPT = `
...
5. When a heading is a contact/coordonn\xE9es block, set fieldKey to "contact" and include any phone, email, address, or links inside that section content.
...
- Map common French headings to the canonical keys, e.g. "Langues", "Langues parl\xE9es" -> "languages"; "Coordonn\xE9es", "Contact" -> "contact"; "Profil", "R\xE9sum\xE9", "\xC0 propos" -> "introduction".
...
EXAMPLE OUTPUT:
{
 "sections": [
   ...
   {
     "title": "Langues",
     "content": "Fran\xE7ais (natif), Anglais (courant)",
     "fieldKey": "languages",
     "confidence": 0.95
   },
   {
...
EXAMPLE OUTPUT (French CV \u2014 few-shot example to improve multilingual behavior):
{
  "sections": [
    {
      "title": "Coordonn\xE9es",
      "content": "[Your Full Name]\\n[Professional Title, e.g., Sp\xE9cialiste en Marketing Digital]\\nPhone: +33 6 XX XX XX XX | Email: prenom.nom@email.com | LinkedIn: linkedin.com/in/prenom-nom | City, France",
      "fieldKey": "contact",
      "confidence": 0.98
    },
    {
      "title": "Profil",
      "content": "Sp\xE9cialiste en marketing digital avec plus de 5 ans d\u2019exp\xE9rience dans la gestion de campagnes en ligne. Expert en SEO et r\xE9seaux sociaux, ayant augment\xE9 l\u2019engagement client de 45 % chez une entreprise pr\xE9c\xE9dente. Passionn\xE9 par l\u2019innovation num\xE9rique, je vise \xE0 contribuer \xE0 des projets dynamiques au sein d\u2019une \xE9quipe collaborative.",
      "fieldKey": "introduction",
      "confidence": 0.96
    },
    {
      "title": "Exp\xE9rience Professionnelle",
      "content": "Sp\xE9cialiste en Marketing Digital\\nEntreprise XYZ, Paris, France\\n09/2022 \u2013 Pr\xE9sent\\n- D\xE9velopp\xE9 et ex\xE9cut\xE9 des strat\xE9gies SEO, augmentant le trafic organique de 30 %.\\n- G\xE9r\xE9 les campagnes sur les r\xE9seaux sociaux, g\xE9n\xE9rant 50 000 \u20AC de ventes suppl\xE9mentaires.\\n- Analys\xE9 les donn\xE9es analytics pour optimiser les performances mensuelles.\\n- Collabor\xE9 avec une \xE9quipe de 8 personnes pour lancer des produits num\xE9riques.\\n\\nAssistant Marketing\\nAgence ABC, Lyon, France\\n06/2020 \u2013 08/2022\\n- Cr\xE9\xE9 du contenu pour les plateformes sociales, augmentant les abonn\xE9s de 20 %.\\n- Assist\xE9 dans la planification d\u2019\xE9v\xE9nements virtuels atteignant 1 000 participants.",
      "fieldKey": "experience",
      "confidence": 0.95
    },
    {
      "title": "Formation",
      "content": "Licence en Commerce et Marketing (Bac+3)\\nUniversit\xE9 de Paris, Paris, France\\n09/2017 \u2013 06/2020\\n- Mention Bien (\xE9quivalent \xE0 GPA 3.5/4.0).\\n- Projet de fin d\u2019\xE9tudes : Analyse de march\xE9 pour une startup tech.",
      "fieldKey": "education",
      "confidence": 0.94
    },
    {
      "title": "Comp\xE9tences",
      "content": "Hard Skills: SEO, Google Analytics, Adobe Photoshop, Microsoft Office.\\nSoft Skills: Travail d\u2019\xE9quipe, Gestion de projets, Communication.\\nInformatique: Ma\xEEtrise de Python et Excel avanc\xE9.",
      "fieldKey": "skills",
      "confidence": 0.93
    },
    {
      "title": "Langues",
      "content": "Fran\xE7ais : Natif (C2)",
      "fieldKey": "languages",
      "confidence": 0.98
    }
  ]
...
"""
`;
var METADATA_EXTRACTION_PROMPT = `
...
var llmConfig = {
  ...
  provider: process.env.LLM_PROVIDER ?? (process.env.MISTRAL_API_KEY ? "mistral" : "openai"),
  // Model selection: central fallback order
  model: process.env.LLM_MODEL ?? process.env.MISTRAL_MODEL ?? process.env.OPENAI_MODEL ?? "mistral-small-latest",
...
// my-app/convex/utils/parseHelpers.ts
function extractLanguages(text) {
  ...
  for (let i2 = 0; i2 < lines.length; i2++) {
    ...
    if (/(^|\W)(langues|languages|langue|language)(\W|$)/i.test(line)) {
      ...
      for (let j2 = i2 + 1; j2 < lines.length; j2++) {
        ...
        if (!l2 || /^\s*$/.test(l2))
          break;
        if (/^[A-Z \-]{2,}$/.test(l2.trim()) || /^[\s#]*[A-Za-zÀ-ÖØ-öø-ÿ0-9 \-]+:/.test(l2.trim()))
          break;
        captured.push(l2.trim());
      }
  ...
  if (bullets && bullets.length >= 2) {
    const items = bullets.map((b2) => b2.replace(/^[\-\*\•]\s*/, "").trim());
    const filtered = items.filter((i2) => i2.length <= 40 && i2.split(" ").length <= 4);
    if (filtered.length >= 2)
      return Array.from(new Set(filtered));
  }
  return [];
}
function parseLanguageListFromBlock(block) {
  if (!block)
    return [];
  const candidates = block.split(/[,\/;\n•\-\*]+/).map((s2) => s2.trim()).filter(Boolean);
  return splitAndNormalizeLanguageList(candidates.join(", "));
}
function splitAndNormalizeLanguageList(s2) {
  const parts = s2.split(/[,\/;]+/).map((p2) => p2.trim()).filter(Boolean);
  const seen = /* @__PURE__ */ new Set();
  const out = [];
  for (const p2 of parts) {
    const cleaned = p2.replace(/\s*\((?:native|natif|fluent|courant|bilingue|intermédiaire|basic|débutant|A\d|B\d|C\d)\)/i, "").trim();
...
async function callLLM(prompt, schema, skipAdapters, opts) {
  ...
  if (!skipAdapters) {
    try {
      ...
      if (typeof getLLMAdapterFn === "function") {
        try {
          const requestedModel = llmConfig?.model ?? llmConfig?.openaiModel ?? null;
          try {
            recordTelemetry("adapter.delegate_attempt", { provider: llmConfig.provider ?? "unknown", requestedModel });
          } catch {
          }
        } catch {
        }
        const adapter = getLLMAdapterFn(llmConfig);
        if (adapter && typeof adapter.call === "function") {
          try {
            const adapterResult = await adapter.call(prompt, schema, opts);
            try {
              recordTelemetry("adapter.delegate_response", { provider: llmConfig.provider ?? "unknown", requestedModel: llmConfig?.model ?? null, success: true });
            } catch {
            }
            return typeof adapterResult === "string" ? adapterResult : JSON.stringify(adapterResult);
          } catch (adapterCallErr) {
            try {
              recordTelemetry("adapter.delegate_response", { provider: llmConfig.provider ?? "unknown", requestedModel: llmConfig?.model ?? null, success: false, error: String(adapterCallErr) });
            } catch {
            }
          }
        }
      ...
      try {
        recordTelemetry("adapter.delegate_response", { provider: llmConfig?.provider ?? "unknown", requestedModel: llmConfig?.model ?? null, success: false, error: String(adapterErr) });
      } catch {
      }
...
IMPORTANT: Return ONLY valid JSON (no markdown, no surrounding explanation).` : prompt;
    ...
    const wantsJson = Boolean(schema) || parsingMarkers.some((m2) => typeof m2 === "string" && prompt.includes(m2));
    if (wantsJson && schema) {
      body.text = {
        format: {
          type: "json_schema",
    ...
    if (body.text?.format?.type === "json_schema") {
      ...
      if (!body.text.format.json_schema.name)
        body.text.format.json_schema.name = "response";
      if (!body.text.format.name)
        body.text.format.name = body.text.format.json_schema.name ?? "response";
      if (!body.text.format.schema)
        body.text.format.schema = body.text.format.json_schema.schema ?? { type: "object", additionalProperties: false };
    }
    try {
      if (body.text?.format) {
        const safeFormat = { ...body.text.format ?? {} };
        if (safeFormat.schema && typeof safeFormat.schema === "object")
          safeFormat.schema = "[REDACTED_SCHEMA]";
      ...
      console.warn("[callLLM][DEBUG] unable to stringify outgoing format:", String(e2));
    }
    try {
      recordTelemetry("adapter.request_shape", { provider: "openai", model: body.model, hasTopLevelSchema: !!body.text?.format?.schema, hasNestedSchema: !!body.text?.format?.json_schema?.schema });
    } catch {
    }
    ...
    if (body.text?.format && !body.text.format.schema) {
      ...
      try {
        recordTelemetry("adapter.request_shape_invalid", { reason: "missing_top_level_schema" });
      } catch {
      }
    ...
    if (!res.ok) {
      ...
      if (res.status === 400 && /model_not_found/i.test(txt)) {
        ...
        try {
          ...
          try {
            console.warn("[callLLM] retry fetch failed:", String(retryErr));
          } catch {
          }
        }
      } else {
    ...
    try {
      console.log("[callLLM] provider response keys:", Object.keys(j2));
    } catch (e2) {
    }
    if (Array.isArray(j2.choices) && j2.choices[0]?.message?.content) {
      const content = j2.choices[0].message.content;
      console.log("[callLLM] detected choices[0].message.content (chat style)");
      return String(content);
    }
    if (j2.full_response && Array.isArray(j2.full_response.choices) && j2.full_response.choices[0]?.message?.content) {
      console.log("[callLLM] detected full_response.choices chat-style");
      return String(j2.full_response.choices[0].message.content);
    }
    const outputs = j2.output || j2.outputs || j2.output?.[0]?.content || j2.outputs?.[0]?.content;
    ...
    if (j2?.output?.[0]?.content?.[0]) {
      const c2 = j2.output[0].content[0];
      if (c2.json) {
    ...
    const asString = JSON.stringify(j2);
...
async function repairJSON(broken, timeoutMs = 2e3, llmCaller = callLLM) {
  try {
...
`;
  ...
  try {
    const res = await Promise.race([
      llmCaller(fixPrompt),
      new Promise((_2, reject) => setTimeout(() => reject(new Error("repair timeout")), timeoutMs))
    ]).catch((e2) => {
      console.warn("[repairJSON] llmCaller rejected:", e2?.message ?? String(e2));
      return null;
    });
    if (!res) {
    ...
    const tryParse = (s2) => {
      ...
      try {
        ...
        console.log("[repairJSON] tryParse failed:", err.message);
        return null;
      }
    };
    ...
    try {
      ...
      let m2;
      while ((m2 = jsonCandidateRegex.exec(res)) !== null) {
    ...
    if (firstBrace >= 0) {
      ...
      let depth = 0;
      for (let i2 = firstBrace; i2 < res.length; i2++) {
        const ch = res[i2];
        if (ch === "{")
          depth++;
        else if (ch === "}") {
          depth--;
          if (depth === 0) {
            const candidate = res.slice(firstBrace, i2 + 1);
            parsed = tryParse(candidate);
            if (parsed) {
...
function detectLanguageIsFrench(text) {
  if (!text)
    return false;
  try {
    ...
    if (Array.isArray(langs) && langs.length) {
      for (const l2 of langs) {
        if (typeof l2 === "string" && /francais|français|french/i.test(l2))
          return true;
      }
    }
  } catch {
  ...
  if (frenchDiacritics.test(text))
    return true;
  const lower = text.toLowerCase();
  const strongMarkers = ["fran\xE7ais", "francais", "monsieur", "madame", "m.", "mme", "n\xE9 le", "n\xE9(e) le"];
  for (const m2 of strongMarkers)
...
async function parseCV(rawText) {
  ...
  let fallbackReason = "";
  try {
    let sanitizeProviderResponse2 = function(raw) {
      if (!raw)
        return raw;
      try {
        ...
        if (parsed && typeof parsed === "object") {
          ...
          if (parsed.metadata && typeof parsed.metadata === "object") {
            if (typeof parsed.metadata.output_text === "string" && parsed.metadata.output_text.trim()) {
              const fence = /```(?:json)?\s*([\s\S]*?)```/i.exec(parsed.metadata.output_text);
              if (fence && fence[1])
                return fence[1].trim();
              return String(parsed.metadata.output_text);
            }
          }
          if (parsed.output && Array.isArray(parsed.output)) {
            for (const item of parsed.output) {
              ...
              if (Array.isArray(item.content) && item.content.length) {
                for (const c2 of item.content) {
                  if (!c2)
                    continue;
                  if (typeof c2 === "string") {
                    const fence = /```(?:json)?\s*([\s\S]*?)```/i.exec(c2);
                    if (fence && fence[1])
                      return fence[1].trim();
                    return String(c2);
                  }
                  if (typeof c2 === "object") {
                    ...
                    if (typeof c2.text === "string") {
                      const fence = /```(?:json)?\s*([\s\S]*?)```/i.exec(c2.text);
                      if (fence && fence[1])
                        return fence[1].trim();
                      return String(c2.text);
                    }
                    if (typeof c2.output_text === "string")
                      return String(c2.output_text);
                  }
                }
              }
            }
          ...
          if (parsed.full_response && Array.isArray(parsed.full_response.choices)) {
            const c2 = parsed.full_response.choices[0];
            if (c2?.message?.content) {
              const content = String(c2.message.content);
      ...
      try {
        ...
        if (fenceUnesc && fenceUnesc[1])
          return fenceUnesc[1].trim();
      ...
      if (keywordIdx !== -1) {
        ...
        if (braceIdx >= 0) {
          let depth = 0;
          for (let i2 = braceIdx; i2 < raw.length; i2++) {
            const ch = raw[i2];
            if (ch === "{")
              depth++;
            else if (ch === "}") {
              depth--;
              if (depth === 0) {
                const candidate = raw.slice(braceIdx, i2 + 1);
                try {
    ...
    var sanitizeProviderResponse = sanitizeProviderResponse2;
    const { parseLLMSections: parseLLMSections2, parseLLMMetadata: parseLLMMetadata2 } = await Promise.resolve().then(() => (init_llmPostProcessor(), llmPostProcessor_exports));
    async function callPreferredProvider(promptWithText, rawText2, timeoutMs, options, schema) {
      const enforcedForceGpt = process.env.FORCE_GPT_ONLY === "1";
      const forceGpt = enforcedForceGpt || Boolean(llmConfig?.forceGpt5NanoOnly);
      const allowProviderFallback = options?.allowProviderFallback ?? true;
      const requestedModel = forceGpt ? llmConfig?.openaiModel ?? "gpt-5-nano" : llmConfig?.mistralModel ?? llmConfig?.model ?? "mistral-small-latest";
      try {
        console.log("LLM plan:", {
          configuredProvider: llmConfig?.provider ?? "unknown",
          requestedModel,
          forceGpt5NanoOnly: !!llmConfig?.forceGpt5NanoOnly,
          enforcedForceViaEnv: enforcedForceGpt,
          allowProviderFallback
        });
      } catch {
      }
      try {
        recordTelemetry("adapter.call_plan", { requestedModel, forceGpt5NanoOnly: forceGpt, enforcedForceGpt });
      } catch {
      }
      if (forceGpt) {
        const model = llmConfig?.openaiModel ?? "gpt-5-nano";
        try {
          recordTelemetry("adapter.provider_attempt", { provider: "openai", attemptedModel: model });
        } catch {
        }
        const start = Date.now();
        const text = await callLLMWithTimeout(promptWithText, rawText2, timeoutMs, schema, true);
        const duration = Date.now() - start;
        try {
          recordTelemetry("adapter.provider_latency", { provider: "openai", attemptedModel: model, durationMs: duration });
        } catch {
        }
        return { text, fallbackUsed: false };
      }
      if (!allowProviderFallback) {
        const fallbackModel2 = llmConfig?.openaiModel ?? "gpt-5-nano";
        try {
          recordTelemetry("adapter.provider_attempt", { provider: "openai", attemptedModel: fallbackModel2 });
        } catch {
        }
        const start = Date.now();
        const text = await callLLMWithTimeout(promptWithText, rawText2, timeoutMs, schema);
        const duration = Date.now() - start;
        try {
          recordTelemetry("adapter.provider_latency", { provider: "openai", attemptedModel: fallbackModel2, durationMs: duration });
        } catch {
        }
        return { text, fallbackUsed: false };
      }
      ...
      try {
        ...
        if (typeof getLLMAdapterFn === "function") {
          try {
            recordTelemetry("adapter.provider_attempt", { provider: llmConfig?.provider ?? "unknown", attemptedModel: requestedModel });
          } catch {
          }
          const adapter = getLLMAdapterFn(llmConfig);
          if (adapter && typeof adapter.call === "function") {
            const start = Date.now();
            try {
              const effectiveTimeout = Math.min(timeoutMs, 2e4);
              const controller = new AbortController();
              const adapterPromise = adapter.call(promptWithText, void 0, { signal: controller.signal });
              let adapterResult;
              try {
                adapterResult = await Promise.race([
                  adapterPromise,
                  new Promise((_2, reject) => setTimeout(() => reject(new Error("adapter timeout")), effectiveTimeout))
                ]);
              } catch (raceErr) {
                try {
                  controller.abort();
                ...
                try {
                  await Promise.race([adapterPromise.catch(() => {
                  }), new Promise((r2) => setTimeout(r2, 2e3))]);
                } catch {
                }
                throw raceErr;
              }
              const duration = Date.now() - start;
              try {
                recordTelemetry("adapter.provider_latency", { provider: llmConfig?.provider ?? "unknown", attemptedModel: requestedModel, durationMs: duration });
              } catch {
              }
              const textResult = typeof adapterResult === "string" ? adapterResult : JSON.stringify(adapterResult);
              try {
                recordTelemetry("adapter.provider_result", { provider: llmConfig?.provider ?? "unknown", attemptedModel: requestedModel, outcome: "ok" });
              } catch {
              }
              return { text: textResult, fallbackUsed: false };
            } catch (adapterErr) {
              const duration = Date.now() - start;
              try {
                recordTelemetry("adapter.provider_latency", { provider: llmConfig?.provider ?? "unknown", attemptedModel: requestedModel, durationMs: duration, error: String(adapterErr) });
              } catch {
              }
              try {
                recordTelemetry("adapter.provider_result", { provider: llmConfig?.provider ?? "unknown", attemptedModel: requestedModel, outcome: "error", error: String(adapterErr) });
              } catch {
              }
            }
          }
        ...
        try {
          recordTelemetry("adapter.provider_result", { provider: llmConfig?.provider ?? "unknown", attemptedModel: requestedModel, outcome: "adapter_import_failed", error: String(err) });
        } catch {
        }
      }
      const fallbackModel = llmConfig?.openaiModel ?? "gpt-5-nano";
      try {
        try {
          recordTelemetry("adapter.fallback_trace", { requestedModel, provider: llmConfig?.provider ?? "mistral", fallbackModel, attemptIndex: 2, outcome: "fallback_to_openai", attemptHistory: [{ provider: llmConfig?.provider ?? "mistral", attemptedModel: requestedModel, outcome: "error" }] });
        } catch {
        }
        try {
          recordTelemetry("adapter.provider_attempt", { provider: "openai", attemptedModel: fallbackModel });
        } catch {
        }
        const start = Date.now();
        const text = await callLLMWithTimeout(promptWithText, rawText2, timeoutMs, schema, true);
        const duration = Date.now() - start;
        try {
          recordTelemetry("adapter.provider_latency", { provider: "openai", attemptedModel: fallbackModel, durationMs: duration });
        } catch {
        }
        fallbackUsed = true;
        try {
          recordTelemetry("adapter.retry_summary", { requestedModel, fallbackTo: "openai", attempts: 1 });
        } catch {
        }
        return { text, fallbackUsed };
      } catch (finalErr) {
        throw finalErr;
      }
    }
    ...
    const METADATA_RESPONSE_SCHEMA = {
      ...
      // Include all keys in 'required' to avoid Responses API schema validation failures.
      required: ["name", "email", "phone", "linkedinUrl"],
      additionalProperties: false
    };
    async function attemptLLMParse(promptTemplate, parserFn, validateOutput, timeoutMs, schema) {
      const attempts = 2;
      let lastErr = null;
      ...
      for (let attempt = 1; attempt <= attempts; attempt++) {
        ...
        try {
          ...
          if (parsed && validateOutput) {
            const v2 = validateOutput(parsed);
            if (v2.isValid && (v2.confidence ?? 1) > 0.45) {
              try {
                if (parsed && typeof parsed === "object")
                  parsed.telemetry = { providerUsed: null, sanitizedForRepair: false, repairReturnedProviderShape: false };
              } catch {
              }
              return parsed;
            }
          ...
          try {
            const sanitizedPayload = sanitizeProviderResponse2(raw);
            ...
            const strongRepairCaller = hasOpenAIKey() ? (p2) => callOpenAIResponsesForRepair(p2, rawText) : callLLM;
            const repaired = await repairJSON(sanitizedPayload, 5e3, strongRepairCaller);
            if (!repaired) {
                          ...
                          try {
                            ...
                            if (typeof val === "string") {
                              ...
                              if (fenced && fenced[1]) {
                                ...
                                if (tryObj && typeof tryObj === "object") {
                                  extracted = tryObj;
                                  break;
                                }
                              }
    ...
    const sectionPrompt = detectLanguageIsFrench(rawText) ? SECTION_EXTRACTION_PROMPT + "\n\nINSTRUCTIONS (FR): R\xE9pondez uniquement en JSON valide (aucune explication). Si le CV est en fran\xE7ais, r\xE9pondez en fran\xE7ais." : SECTION_EXTRACTION_PROMPT;
    const sectionTimeout = detectLanguageIsFrench(rawText) ? 3e4 : 15e3;
    const parsedSections = await attemptLLMParse(
      sectionPrompt,
      parseLLMSections2,
      (p2) => validateLLMOutput(p2, rawText),
      sectionTimeout,
      SECTION_RESPONSE_SCHEMA
    );
    const metadataPrompt = detectLanguageIsFrench(rawText) ? METADATA_EXTRACTION_PROMPT + "\n\nINSTRUCTIONS (FR): R\xE9pondez uniquement en JSON valide (aucune explication). Si le CV est en fran\xE7ais, r\xE9pondez en fran\xE7ais." : METADATA_EXTRACTION_PROMPT;
    const metadataTimeout = detectLanguageIsFrench(rawText) ? 15e3 : 7e3;
    const parsedMetadata = await attemptLLMParse(
      metadataPrompt,
      parseLLMMetadata2,
      null,
      // no heavy validator; parser returns final shape or null
      metadataTimeout,
    ...
    if (validation.isValid && validation.confidence > 0.7) {
      return {
        sections: parsedSections.sections.map((s2) => ({
          ...s2,
          confidence: s2.confidence * validation.confidence
          // Adjust confidence based on overall validation
        })),
      ...
      if (parsedSections && Array.isArray(parsedSections.sections) && parsedSections.sections.length > 0) {
        ...
        return {
          sections: parsedSections.sections.map((s2) => ({
            ...s2,
            confidence: Math.max(0.4, (s2.confidence ?? 0.5) * (validation.confidence || 0.5))
            // accept but degrade confidence
          })),
          metadata: parsedMetadata,
          method: "llm",
          warnings
...
async function callOpenAIResponsesForRepair(prompt, text, opts) {
  const apiKey = process.env.OPENAI_API_KEY ?? llmConfig.openaiKey ?? null;
  const promptWithText = prompt.replace("{{cvText}}", text);
  if (!apiKey)
    throw new Error("No OpenAI key available for strong repair");
  const modelForRepair = llmConfig?.openaiModel ?? process.env.OPENAI_MODEL ?? "gpt-5-nano";
  ...
  try {
    const openaiModule = await Promise.resolve().then(() => (init_openai(), openai_exports)).catch(() => null);
    const OpenAI2 = openaiModule?.default ?? openaiModule?.OpenAI ?? null;
    if (OpenAI2) {
      try {
        recordTelemetry("adapter.sdk_attempt", { model: modelForRepair });
      } catch {
      }
      ...
      try {
        ...
        try {
          recordTelemetry("adapter.sdk_response", { keys: Object.keys(resp || {}) });
        } catch {
        }
        const j3 = resp;
        if (Array.isArray(j3.choices) && j3.choices[0]?.message?.content)
          return String(j3.choices[0].message.content);
        if (j3.full_response && Array.isArray(j3.full_response.choices) && j3.full_response.choices[0]?.message?.content)
          return String(j3.full_response.choices[0].message.content);
        const outputs2 = j3.output || j3.outputs || j3.output?.[0]?.content || j3.outputs?.[0]?.content;
        ...
        if (j3?.output?.[0]?.content?.[0]) {
          const c2 = j3.output[0].content[0];
          if (c2.json)
            return JSON.stringify(c2.json);
          if (c2.text)
            return String(c2.text);
        ...
        try {
          const asString = JSON.stringify(j3);
        ...
        try {
          recordTelemetry("adapter.sdk_error", { message: msg });
        } catch {
        }
        if (/model_not_found/i.test(msg)) {
          try {
            recordTelemetry("adapter.fallback_trace", { requestedModel: modelForRepair, provider: "openai-sdk", attemptedModel: fallbackModelRepair, attemptIndex: 2, outcome: "fallback_retriable" });
          } catch {
          }
          try {
            const retryReq = { ...req, model: fallbackModelRepair };
            const retryResp = await client.responses.create(retryReq);
            try {
              recordTelemetry("adapter.sdk_response", { keys: Object.keys(retryResp || {}), retriedWith: fallbackModelRepair });
            } catch {
            }
            const jr = retryResp;
            if (Array.isArray(jr.choices) && jr.choices[0]?.message?.content)
              return String(jr.choices[0].message.content);
            ...
            try {
              recordTelemetry("adapter.sdk_error", { message: String(retryErr) });
            } catch {
            }
          }
        }
    ...
    try {
      recordTelemetry("adapter.sdk_import_failed", { message: String(sdkImportErr?.message ?? sdkImportErr) });
    } catch {
    }
  }
  const body = {
  ...
  if (res.status === 429) {
    ...
    const delayMs = retryAfter ? Math.max(1e3, parseInt(retryAfter, 10) * 1e3) : 1e3;
    try {
      recordTelemetry("adapter.retry_summary", { provider: "openai", attemptedModel: modelForRepair, reason: "rate_limited", retryAfterMs: delayMs });
    } catch {
    }
  ...
  if (!res.ok) {
    ...
    if (res.status === 400 && /model_not_found/i.test(bodyText)) {
      try {
        console.warn("[callOpenAIResponsesForRepair] model not found:", body.model, " -> falling back to", fallbackModelRepair);
      } catch {
      }
      try {
        ...
        try {
          console.warn("[callOpenAIResponsesForRepair] retry failed:", String(retryErr));
        } catch {
        }
      }
      if (!res.ok) {
  ...
  try {
    recordTelemetry("adapter.provider_latency", { provider: "openai", attemptedModel: modelForRepair, durationMs: duration });
  } catch {
  }
  const j2 = await res.json();
  if (Array.isArray(j2.choices) && j2.choices[0]?.message?.content)
    return String(j2.choices[0].message.content);
  if (j2.full_response && Array.isArray(j2.full_response.choices) && j2.full_response.choices[0]?.message?.content)
    return String(j2.full_response.choices[0].message.content);
  const outputs = j2.output || j2.outputs || j2.output?.[0]?.content || j2.outputs?.[0]?.content;
  if (Array.isArray(outputs)) {
...
async function callLLMWithTimeout(prompt, text, timeoutMs, schema, skipAdapters) {
  const promptWithText = prompt.replace("{{cvText}}", text);
  const effectiveTimeout = Math.min(timeoutMs, 2e4);
  const controller = new AbortController();
  ...
  try {
    ...
    try {
      await Promise.race([llmPromise.catch(() => {
      }), new Promise((r2) => setTimeout(r2, 2e3))]);
    } catch {
    }
    throw err;
...
function parseWithEnhancedHeuristics(text) {
  const lines = text.split("\n");
  const sections = [];
  let currentSection = null;
  const determineFieldKey = (line) => "unknown";
  for (let i2 = 0; i2 < lines.length; i2++) {
    const line = lines[i2];
    const context = {
      previousLine: i2 > 0 ? lines[i2 - 1] : "",
      nextLine: i2 < lines.length - 1 ? lines[i2 + 1] : "",
      lineIndex: i2
    };
    if (isPotentialHeader(line, context)) {
      ...
      currentSection = {
        ...
        confidence: 0.7
        // Base confidence for heuristic parsing
      };
...
// my-app/scripts/run-single-job.mts
var import_fs2 = __toESM(require("fs"), 1);
async function main() {
  process.env.FORCE_GPT_ONLY = "1";
  const sampleFrenchCV = `
PROFIL
Ing\xE9nieur logiciel exp\xE9riment\xE9 avec 5 ans d'exp\xE9rience dans le d\xE9veloppement backend.

EXP\xC9RIENCE
2020 - 2024 \u2014 Acme Corp
- Ing\xE9nieur logiciel senior
- D\xE9veloppement de services Node.js et TypeScript
...
https://linkedin.com/in/johndoe
  ...
  try {
    ...
    try {
      import_fs2.default.writeFileSync("tmp-parse-result.json", JSON.stringify(result, null, 2), "utf-8");
