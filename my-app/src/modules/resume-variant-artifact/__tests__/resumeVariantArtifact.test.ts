...
function graph(overrides: Partial<EvidenceGraphV1> = {}): EvidenceGraphV1 {
  return {
    ...
    matches: [
      {
        id: "evidence-match:typescript-direct",
        demandId: "demand:typescript-required",
        candidateFactId: "candidate-fact:typescript-skill",
        sourceDocumentId: "candidate-source-document:skills-source",
        sourcePath: "document.skills[0]",
        matchType: "direct",
        strength: "strong",
        reviewState: "accepted",
        reason: "Source-backed match.",
        version: 1,
      },
    ],
    missing: [],
    ...
    allowedClaims: [
      {
        id: "allowed-claim:typescript-skill",
        candidateFactIds: ["candidate-fact:typescript-skill"],
        claimType: "skill",
        text: "TypeScript",
        supportLevel: "strong",
        reviewState: "allowed",
        reason: "Approved source-backed fact.",
        version: 1,
      },
    ],
    blockedClaimIds: [],
...
function plan(
  evidenceGraph = graph(),
  overrides: Partial<ResumeVariantPlanV1> = {},
): ResumeVariantPlanV1 {
  return {
    ...
    items: [
      {
        id: "resume-variant-plan-item:skills:typescript",
        section: "skills",
        ...
        candidateFactIds: ["candidate-fact:typescript-skill"],
        evidenceMatchIds: ["evidence-match:typescript-direct"],
        demandIds: ["demand:typescript-required"],
        riskFlagIds: [],
        reason: "Use source-backed claim.",
        version: 1,
      },
    ],
    warnings: [],
...
function cockpit(
  evidenceGraph = graph(),
  resumeVariantPlan = plan(evidenceGraph),
  overrides: Partial<ReviewCockpitModelV1> = {},
  ...
  return {
    ...
    items: [
      {
        id: "review-cockpit-item:source-support:typescript",
        bucket: "source_support",
        title: "Allowed claim has source support",
        description: "Allowed claim has source-backed support.",
        severity: "info",
        planItemId: "resume-variant-plan-item:skills:typescript",
        allowedClaimId: "allowed-claim:typescript-skill",
        candidateFactId: "candidate-fact:typescript-skill",
        evidenceMatchId: "evidence-match:typescript-direct",
        demandId: "demand:typescript-required",
        sourceFactIds: ["candidate-fact:typescript-skill"],
        allowedClaimIds: ["allowed-claim:typescript-skill"],
        riskFlagIds: [],
...
function input(
  overrides: Partial<BuildResumeVariantArtifactInputV1> = {},
): BuildResumeVariantArtifactInputV1 {
...
function linkedInput(
  options: {
    evidenceGraph?: Partial<EvidenceGraphV1>;
    resumeVariantPlan?: Partial<ResumeVariantPlanV1>;
    reviewCockpit?: Partial<ReviewCockpitModelV1>;
    input?: Partial<BuildResumeVariantArtifactInputV1>;
  } = {},
): BuildResumeVariantArtifactInputV1 {
...
function mutateFirstSourceBackedItem(
  artifact: ResumeVariantArtifactV1,
  mutate: (item: ResumeVariantArtifactV1["sections"][number]["items"][number]) => ResumeVariantArtifactV1["sections"][number]["items"][number],
): ResumeVariantArtifactV1 {
  return {
    ...artifact,
    sections: artifact.sections.map((section) => ({
      ...section,
      items: section.items.map((item) => (item.kind === "source_backed_claim" ? mutate(item) : item)),
    })),
  };
}
...
describe("resume variant artifact", () => {
  it("builds deterministic ID, content, source-backed item, provenance, and sorted collectors", async () => {
    ...
    expect(collectResumeVariantArtifactReviewItemIds(first)).toEqual([
      "review-cockpit-item:source-support:typescript",
    ]);
  });
  ...
  it("same source chain with different timestamps produces the same artifact hash", async () => {
    ...
    await expect(buildResumeVariantArtifactHash(rebuiltLater)).resolves.toBe(
      await buildResumeVariantArtifactHash(source),
    );
  });
  ...
  it("same source chain with different timestamps produces the same artifact ID", async () => {
    ...
    await expect(buildResumeVariantArtifact(rebuiltLater)).resolves.toMatchObject({
      id: (await buildResumeVariantArtifact(source)).id,
      createdAt: T + 10_000,
      updatedAt: T + 20_000,
    });
  });
  ...
  it("changes hash when EvidenceGraph, ResumeVariantPlan, or ReviewCockpit changes", async () => {
    ...
    await expect(
      buildResumeVariantArtifactHash(linkedInput({ evidenceGraph: { id: "evidence-graph:changed" } })),
    ).resolves.not.toBe(base);
    await expect(
      buildResumeVariantArtifactHash(linkedInput({ resumeVariantPlan: { id: "resume-variant-plan:changed" } })),
    ).resolves.not.toBe(base);
    await expect(
      buildResumeVariantArtifactHash(linkedInput({ reviewCockpit: { id: "review-cockpit:changed" } })),
    ).resolves.not.toBe(base);
  });
  ...
  it("derives blocked, needs_review, and draft statuses", async () => {
    const blocked = linkedInput({
      resumeVariantPlan: { blocked: true, blockedReason: "manual" },
      reviewCockpit: { summary: { ...cockpit().summary, status: "blocked", blockerCount: 1 } },
    });
    const needsReview = linkedInput({
      reviewCockpit: { summary: { ...cockpit().summary, status: "needs_review", warningCount: 1 } },
    });
    const emptyGraph = graph();
    const emptyPlan = plan(emptyGraph, { items: [], sourceFactIds: [], allowedClaimIds: [] });
    const draft = input({
      evidenceGraph: emptyGraph,
      resumeVariantPlan: emptyPlan,
      reviewCockpit: cockpit(emptyGraph, emptyPlan, {
        items: [],
        summary: {
          ...cockpit().summary,
          allowedClaimCount: 0,
  ...
  it("preserves notice provenance for missing evidence, warnings, blockers, and risks", async () => {
    const evidenceGraph = graph({
      riskFlags: [
        {
          id: "risk:private-source-fact",
          category: "private_fact",
          severity: "blocker",
          candidateFactId: "candidate-fact:typescript-skill",
          reason: "Private fact excluded.",
          version: 1,
        },
      ],
      blockedClaimIds: ["blocked-claim:candidate-fact:typescript-skill"],
    });
    const resumeVariantPlan = plan(evidenceGraph, {
      blocked: true,
      riskFlagIds: ["risk:private-source-fact"],
      warnings: [
        {
          id: "warning:private-source-fact",
          category: "private_fact",
          severity: "blocker",
          riskFlagId: "risk:private-source-fact",
          candidateFactId: "candidate-fact:typescript-skill",
          reason: "Private fact.",
          version: 1,
        },
    ...
    const reviewCockpit = cockpit(evidenceGraph, resumeVariantPlan, {
      summary: { ...cockpit().summary, status: "blocked", blockerCount: 1, riskFlagCount: 1 },
      items: [
        {
          ...cockpit().items[0]!,
          severity: "blocker",
    ...
    const artifact = await buildResumeVariantArtifact(
      input({ evidenceGraph, resumeVariantPlan, reviewCockpit }),
    );
  ...
  it("evidence assertion rejects source-backed items without allowed claims or source facts", async () => {
    ...
    const withoutClaim = mutateFirstSourceBackedItem(artifact, (item) => ({
      ...item,
      allowedClaimId: undefined,
      allowedClaimIds: [],
    }));
    const withoutFact = mutateFirstSourceBackedItem(artifact, (item) => ({
      ...item,
      candidateFactId: undefined,
      sourceFactIds: [],
    }));
  ...
  it("provenance consistency rejects unknown allowedClaimId", async () => {
    const source = input();
    const artifact = await buildResumeVariantArtifact(source);
    const invalid = mutateFirstSourceBackedItem(artifact, (item) => ({
      ...item,
      allowedClaimId: "allowed-claim:unknown",
      allowedClaimIds: ["allowed-claim:unknown"],
    }));

    expect(() => assertResumeVariantArtifactEvidenceBacked(invalid, source)).toThrow(TypeError);
  });

  it("provenance consistency rejects unknown evidenceMatchId", async () => {
    const source = input();
    const artifact = await buildResumeVariantArtifact(source);
    const invalid = mutateFirstSourceBackedItem(artifact, (item) => ({
      ...item,
      evidenceMatchId: "evidence-match:unknown",
      evidenceMatchIds: ["evidence-match:unknown"],
    }));

    expect(() => assertResumeVariantArtifactEvidenceBacked(invalid, source)).toThrow(TypeError);
  });

  it("provenance consistency rejects unknown riskFlagId", async () => {
    const source = input();
    const artifact = await buildResumeVariantArtifact(source);
    const invalid = mutateFirstSourceBackedItem(artifact, (item) => ({
      ...item,
      riskFlagId: "risk:unknown",
      riskFlagIds: ["risk:unknown"],
    }));

    expect(() => assertResumeVariantArtifactEvidenceBacked(invalid, source)).toThrow(TypeError);
  });

  it("provenance consistency rejects unknown demandId", async () => {
    const source = input();
    const artifact = await buildResumeVariantArtifact(source);
    const invalid = mutateFirstSourceBackedItem(artifact, (item) => ({
      ...item,
      demandId: "demand:unknown",
      demandIds: ["demand:unknown"],
    }));
