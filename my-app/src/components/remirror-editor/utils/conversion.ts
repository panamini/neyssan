/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/no-redundant-type-constituents, @typescript-eslint/no-unnecessary-type-assertion, @typescript-eslint/no-unsafe-argument, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-return, @typescript-eslint/no-unused-vars -- Existing lint debt is captured locally for this release-gate baseline; fix these rules in focused follow-ups. */
import {
  isRemirrorJSON,
  RemirrorJSON,
} from '@remirror/core';
    ...
    .map((node: any) => {
      ...
      if (Array.isArray(rawStructured) && rawStructured.length > 0 && sectionType === 'languages') {
       ...
       if (Array.isArray(rawStructured) && rawStructured.length > 0 && sectionType === 'achievements') {
        const structured = rawStructured.map((it: any, idx: number) => {
          const rawItemId = String(it?.id ?? '').trim();
          const itemId = rawItemId !== '' ? rawItemId : `ach-${id}-${idx}`;
          const text =
            typeof it === 'string'
              ? it
              : typeof it?.text === 'string'
              ? it.text
              : typeof it?.achievement === 'string'
              ? it.achievement
              : String(it ?? '');
          return {
            id: itemId,
...
const emptyDoc: RemirrorJSON = {
  type: 'doc',
  // Empty editor doc — do NOT seed visible placeholder text into the stored document.
  // The Remirror PlaceholderExtension will provide the UI-only guidance.
