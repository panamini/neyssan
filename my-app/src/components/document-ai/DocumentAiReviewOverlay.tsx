...
import {
  ...
  type DocumentAiSurfacePlacement,
...
export type DocumentAiReviewTarget = {
  sectionId: string;
  sectionType: string;
  sectionLabel: string;
  itemId?: string;
...
export type DocumentAiReviewState = "loading" | "ready" | "error" | "accepted";

export type DocumentAiReviewOverlayProps = {
  open: boolean;
  target: DocumentAiReviewTarget;
  state: DocumentAiReviewState;
  beforeText: string;
  afterText: string;
  errorMessage?: string;
  actionId?: string;
  ...
  preferredSurfacePosition?: DocumentAiSurfacePosition | null;
...
function buildTargetLabel(target: DocumentAiReviewTarget): string {
...
function estimateDocumentAiReviewSurfaceHeight({
  state,
  afterText,
  errorMessage,
}: {
...
export function DocumentAiReviewOverlay({
  open,
  target,
  state,
  beforeText,
  ...
  const surfaceState =
    state === "loading"
      ...
      : state === "accepted"
        ? "applied"
        : "result";
  const estimatedSurfaceHeight = estimateDocumentAiReviewSurfaceHeight({
    state,
    afterText,
    errorMessage,
  });
  const surfacePosition = useDocumentAiSurfacePosition({
    anchor,
    desiredSurfaceSize: {
...
export default DocumentAiReviewOverlay;
