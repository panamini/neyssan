...
describe("ProposalDisplay CSS contracts", () => {
  ...
  it("keeps Proposal toolbar button geometry tied to action hierarchy", () => {
    ...
    expect(productCss).toMatch(
      /\.dasti-proposal-skeleton-stage\[data-ask-mode="iconOnly"\][\s\S]*?\.dasti-proposal-skeleton-stage__ask-handle,[\s\S]*?\.dasti-proposal-skeleton-stage\[data-ask-mode="edgeTab"\][\s\S]*?\.dasti-proposal-skeleton-stage__ask-handle\s*\{[\s\S]*inline-size:\s*calc\(var\(--control-sm\)\s*\+\s*var\(--space-1\)\);[\s\S]*border-color:\s*transparent;[\s\S]*background:\s*color-mix\([\s\S]*var\(--proposal-document-paper\)\s*88%,[\s\S]*var\(--paper-dark-body-ink\)\s*12%[\s\S]*\);[\s\S]*box-shadow:\s*none;/,
    );
    expect(productCss).toMatch(
