...
describe("ProposalDisplay CSS contracts", () => {
  ...
  it("lifts the live output toolbar out of the document shell flow so the A4 stage keeps its full block budget", () => {
    expect(productCss).toMatch(
      /\.dasti-proposal-output-shell\s*\{[\s\S]*--document-shell-padding-inline:\s*8px;[\s\S]*--document-shell-padding-block:\s*8px;/,
    );
    expect(productCss).toMatch(
      /\.dasti-proposal-output-shell--workspace\s*\{[\s\S]*--document-shell-padding-inline:\s*4px;/,
    );
    expect(productCss).toMatch(
    ...
    expect(productCss).toMatch(
      /\.dasti-proposal-output-shell\s+\.dasti-document-rail\s*\{[\s\S]*position:\s*absolute;[\s\S]*inset-block-start:\s*calc\(-1 \* var\(--proposal-output-toolbar-lift\)\);/,
    );
    expect(productCss).toMatch(
    ...
    expect(productCss).toMatch(
      /\.dasti-proposal-output-shell[\s\S]*?\.dasti-proposal-sheet__body--document-viewer[\s\S]*?\.dasti-document-stage-chassis\s*\{[\s\S]*justify-content:\s*center;[\s\S]*padding-block:\s*8px;[\s\S]*padding-inline:\s*8px;/,
    );
    expect(productCss).toMatch(
  ...
  it("lets the desktop Proposal Forge compose shell match the live output shell block size instead of shrinking by A4 ratio", () => {
    expect(productCss).toMatch(
      /\.dasti-proposal-compose-column--workspace\s*\{[\s\S]*--document-shell-padding-inline:\s*4px;[\s\S]*--proposal-sheet-margin-inline-inner:\s*4px;[\s\S]*--proposal-sheet-margin-inline-outer:\s*4px;[\s\S]*--proposal-live-shell-block-size:\s*var\(--proposal-workspace-shell-block-size\);/,
    );
    expect(productCss).toMatch(
