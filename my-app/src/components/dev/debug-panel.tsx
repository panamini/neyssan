...
export function DebugPanel() {
  ...
  return (
    <div
      aria-hidden="false"
      className="fixed left-4 top-4 z-[9999] w-[min(720px,95vw)] max-h-[70vh] overflow-hidden rounded-md shadow-lg bg-white dark:bg-slate-900 border border-neutral-200 dark:border-slate-700"
    >
      <div className="flex items-center justify-between px-3 py-2 border-b dark:border-slate-700">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => clearLogs()}
            className="px-2 py-1 text-xs rounded bg-neutral-100 dark:bg-slate-800"
          >
            Clear
          </button>
          <button
...
function DebugPanelDebugToggle() {
  ...
  return (
    <button
      type="button"
      onClick={toggleDebug}
      className={`px-2 py-1 text-xs rounded ${enabled ? "bg-red-600 text-white" : "bg-neutral-100 dark:bg-slate-800"}`}
