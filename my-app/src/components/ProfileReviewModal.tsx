...
export default function ProfileReviewModal({ visible, parsedProfile, onClose, onSaved }: IProfileReviewProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={(e) => { e.stopPropagation(); if (e.target === e.currentTarget) onClose(); }} style={{ background: 'hsla(30,12%,11%,.32)', backdropFilter: 'blur(8px)' }}>
      <div className="[background:var(--sfr)] w-full max-w-4xl mx-auto rounded-rl [box-shadow:var(--shc)] p-4 md:p-6 overflow-hidden flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
