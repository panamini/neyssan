  ...
  return (
    <div className="w-full">
      <div className="flex items-start justify-between mb-2">
        <h4 className="text-sm font-medium text-muted dark:text-gray-300">{label}</h4>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={`Clear ${label}`}
            onClick={onClear}
            className="p-1 rounded text-muted hover:bg-surface dark:hover:bg-gray-800 dark:text-gray-400"
          >
      ...
      <div className="flex flex-col gap-4">
          ...
          {isEditing ? (
            ...
            displayMode === "chips" ? (
              ...
              <textarea
                ...
                rows={6}
                className="w-full p-2 font-sans text-sm text-gray-900 border rounded-md bg-background dark:bg-surface dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-accent"
              />
            )
          ) : displayMode === "chips" ? (
        ...
        {/* Right: Suggestion */}
        <Card className="relative flex-1 bg-background dark:bg-surface">
          <label className="sr-only">{label} (Suggestion)</label>

          {isLoading ? (
            <div className="flex items-center justify-center h-full">
              <LoadingSpinner />
              <span className="ml-2 text-sm text-muted dark:text-gray-300">Refining...</span>
            </div>
          ) : (
            <>
              ...
              {suggestion ? (
                displayMode === "chips" ? (
                  ...
                  <div className="h-full min-h-[96px] overflow-auto text-sm text-foreground dark:text-gray-100 whitespace-pre-wrap transition-opacity duration-300 ease-in">
                    {suggestion}
                  </div>
                )
              ) : (
                <div className="h-full min-h-[96px] flex items-center justify-center text-sm text-muted dark:text-gray-400">
                  No suggestion
                </div>
              )}
              ...
              <div className="absolute flex flex-col gap-2 top-2 right-2">
                <Button
                  ...
                  onClick={onAccept}
                  disabled={!suggestion}
                  className="px-2 py-1 text-sm rounded-md text-background bg-surface hover:opacity-95 focus:outline-none focus:ring-2 focus:ring-accent"
                >Accept</Button>
                <Button
                  ...
                  onClick={onDiscard}
                  disabled={!suggestion}
                  className="px-2 py-1 text-sm rounded-md bg-surface-muted dark:bg-surface-muted hover:opacity-95 dark:hover:opacity-95 focus:outline-none"
                >Discard</Button>
                {onUndo && (
                  <Button
                    ...
                    onClick={onUndo}
                    className="px-2 py-1 text-sm rounded-md bg-surface text-muted hover:opacity-95 focus:outline-none focus:ring-2 focus:ring-accent"
                  >
                    Undo
                  </Button>
                )}
      ...
      {isEditing && (
        <div className="flex justify-end gap-2 mt-2">
          ...
          <Button
            type="button"
            onClick={applyEdit}
            className="px-3 py-1 text-sm rounded-md text-background bg-accent hover:opacity-95 focus:outline-none focus:ring-2 focus:ring-accent"
          >Save</Button>
        </div>
      )}
    </div>
