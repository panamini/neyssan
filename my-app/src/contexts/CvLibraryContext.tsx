...
import { generateCvTemplate, generateCvTemplateV1, makeExperienceItem, makeEducationItem } from "../lib/cv-template";
import { normalizeAndValidateCvDocument, ensureRepresentativeBlocks } from "../lib/normalize-cv";
...
export interface ICvLibraryContext {
  ...
  openInspector: (payload: { sectionId: string; block: CvBlock; linkedStructured?: Record<string, any> }) => void;
  closeInspector: () => void;
...
export const CvLibraryProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const openInspector = useCallback((payload: { sectionId: string; block: CvBlock; linkedStructured?: Record<string, any> }) => {
    try {
      const linkedIdAttr = (() => {
        try {
...
const flushPendingEdits = useCallback((): void => {
  ...
  function cacheDocumentLocally(doc: CvDocument) {
    try {
      if (typeof window !== "undefined" && window.localStorage) {
        window.localStorage.setItem(`${LOCAL_DOC_KEY_PREFIX}${doc.id}`, JSON.stringify(doc));
      }
    } catch {
      // ignore
    }
    ...
    const p = new Promise<void>((resolve) => {
      saveTimeoutRef.current = setTimeout(async () => {
        ...
        try {
          ...
          // The UI state should not be updated as a side effect of saving.
          // Only update the lastSavedRef to mark the new clean state.
          // The cvs array and currentCv are updated by user actions.

          // Instrumentation: log save boundaries and metadata keys to the in-app debug stream.
          try {
            const stack = (new Error("save-call-stack")).stack?.split("\n").slice(0, 4).map((s) => s.trim());
            // eslint-disable-next-line no-console
            dbg("[CvLibraryContext] scheduleSave invoking adapter.save", {
              docId: docCopy.id,
          ...
          // Call adapter.save (it performs strict validation internally)
          // Mark that a save is in-flight so the isDirty/save effect doesn't re-enter.
          isSavingRef.current = true;
          try {
            await adapter.save(docCopy as any);
            // On success, update lastSavedRef with a content-only snapshot (exclude volatile metadata)
            try {
              // Keep legacy cvState in the saved snapshot for dirty detection,
              // even though we strip it from the payload persisted to the backend.
              // Without this, isDirty would remain true after a save triggered by cvState edits.
              const withLegacy = {
                ...(docCopy as any),
                cvState: (documentToSave as any)?.cvState ?? (currentCv as any)?.cvState,
              };
              lastSavedRef.current = stripMetadata(withLegacy) as CvDocument | null;
              dbg("[CvLibraryContext] scheduleSave: lastSavedRef updated (metadata stripped, cvState preserved for dirty detection)", { docId: docCopy.id });
            } catch {
              // Fallback: if strip fails, keep the full snapshot (best-effort)
              lastSavedRef.current = { ...(docCopy as any), cvState: (currentCv as any)?.cvState } as any;
            }
          } finally {
            // Always clear the saving guard so subsequent saves may be scheduled.
            isSavingRef.current = false;
          }

          // Ensure in-memory currentCv metadata reflects the saved snapshot so dirty detection clears.
          try {
            if (typeof setCurrentCv === "function" && currentCv && String(currentCv.id) === String(docCopy.id)) {
              // Only update metadata when it actually differs to avoid unnecessary re-renders.
              setCurrentCv((prev) => {
                try {
                  if (!prev) return prev;
                  ...
                  if (!deepEqual(prevMeta, savedMeta)) {
                    ...
                    // eslint-disable-next-line no-console
                    dbg("[CvLibraryContext] scheduleSave: synced currentCv.metadata with saved snapshot", { docId: docCopy.id });
                    return patched;
                  } else {
                    dbg("[CvLibraryContext] scheduleSave: metadata is equal, not syncing", { docId: docCopy.id });
                    return prev;
                  }
                } catch {
                  /* noop */
          ...
          // Also cache the successfully saved document to localStorage
          cacheDocumentLocally(docCopy);
        } catch (err) {
          // adapt gracefully: cache locally and continue. Errors are logged client-side.
          // eslint-disable-next-line no-console
  ...
  const loadCv = useCallback((id: string): boolean => {
    ...
    try {
      ...
      if (doc) {
        ...
        const docNorm = ensureRepresentativeBlocks(doc as CvDocument);
        ...
        (async () => {
          try {
            const remoteLoaded = await adapter.load(id);
            ...
            const remoteNorm = ensureRepresentativeBlocks(migratedRemote);
      ...
      (async () => {
        try {
          ...
          try {
            remoteDoc = await adapter.load(id);
            if (remoteDoc) {
              try {
          ...
          if (remoteDoc) {
            const docNorm = ensureRepresentativeBlocks(remoteDoc as CvDocument);
            safeSetCurrentCv(docNorm);
            setCvs((prev) => {
              const exists = prev.some((c) => c.id === docNorm.id);
              if (exists) return prev.map((c) => (c.id === docNorm.id ? docNorm : c));
   ...
   */
  function createNewCv(title?: string) {
    try {
      const cvRaw = isV1SectionsEnabled() ? generateCvTemplateV1(title) : generateCvTemplate(title);
      const cv = {
  ...
  async function importCv(doc: unknown): Promise<void> {
    try {
      ...
      const validatedWithReps = ensureRepresentativeBlocks(validated);
  ...
  // removed duplicate unsafe block left by a previous replacement
    ...
    function updateStructuredItem(sectionId: string, itemId: string, patch: Partial<Record<string, any>>) {
      ...
      const next: CvDocument = { ...currentCv, sections: nextSections };
  ...
  function reorderBlocks(sectionId: string, newOrder: CvBlock[]) {
    ...
    function addSection(section: CvSection) {
      ...
      setCurrentCv((prev) => {
        ...
        // Ensure typed sections have a minimal structuredContent skeleton so the UI renders cards + inspector.
        try {
          const t = String((toInsert as any).type ?? "").toLowerCase().trim();
          const isTyped = t === "experience" || t === "education";
          if (isTyped && !Array.isArray((toInsert as any).structuredContent)) {
            const seedItem = t === "experience" ? (makeExperienceItem as any)() : (makeEducationItem as any)();
            toInsert = { ...(toInsert as any), structuredContent: [seedItem] } as CvSection;
            dbg("[CvLibraryContext] addSection: seeded structuredContent for typed section", {
              sectionId: String(toInsert.id),
              type: t,
              seedItemId: String((seedItem as any)?.id),
            });
          }
        } catch {
          /* non-fatal */
    ...
    function reorderSections(newOrder: CvSection[]) {
      ...
      setCurrentCv((prev) => {
        if (!prev) return prev;
        const next: CvDocument = { ...prev, sections: [...newOrder] };
        return next;
      });
    }
