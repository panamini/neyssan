...
export async function extractTextFromPdf(arrayBuffer: ArrayBuffer): Promise<string> {
  ...
  if (!fullText || fullText.length < TEXT_EXTRACTION_THRESHOLD) {
    ...
    try {
      // Create a worker; the worker internally uses web workers for heavy lifting.
      ...
      let ocrText = "";
      // OCR each page by rendering it into a canvas at higher scale
      for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
        const page = await pdf.getPage(pageNum);
        ...
        // Recognize page using tesseract worker
        // Note: recognize accepts canvas directly in browser environments
        const { data } = await worker.recognize(canvas);
        const pageOcr = (data && data.text) ? data.text.trim() : "";
        ocrText += pageOcr + "\n\n";
      }

      await worker.terminate();
      // If OCR produced text, use it; otherwise keep whatever pdfjs returned
      if (ocrText && ocrText.trim().length > 0) {
        fullText = ocrText.trim();
      }
    } catch (ocrErr) {
      // OCR failed for some reason — log and continue with pdfjs result
      // Do not throw; parsing should be best-effort in the browser.
