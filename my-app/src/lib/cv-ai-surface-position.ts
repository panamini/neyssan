...
export type CvAiSurfacePlacement =
  | "above"
  | "below"
  | "right"
  | "left"
...
export type CvAiSurfaceState = "toolbar" | "loading" | "result" | "applied";
export type CvAiSurfaceMode = "popover" | "sheet";
export type CvAiSurfaceBreakpoint = "desktop" | "narrow";

export type CvAiSurfaceRect = {
  left: number;
  top: number;
  right: number;
  bottom: number;
...
export type CvAiSurfacePosition = {
  placement: CvAiSurfacePlacement;
  left: number;
  top: number;
  maxWidth: number;
  maxHeight: number;
  clamped: boolean;
  mode: CvAiSurfaceMode;
};

export type CvAiSurfacePositionInput = {
  anchor?: EditorSelectionAnchor | null;
  visibleStageRect?: CvAiSurfaceRect | null;
  paperRect?: CvAiSurfaceRect | null;
  topIslandRect?: CvAiSurfaceRect | null;
  leftDrawerRect?: CvAiSurfaceRect | null;
  viewportRect: CvAiSurfaceRect;
  desiredSurfaceSize: CvAiSurfaceSize;
  mode: CvAiSurfaceState;
  breakpoint: CvAiSurfaceBreakpoint;
...
export function cvAiSurfaceRectFromDom(
...
function normalizeRect(rect: CvAiSurfaceRect | null | undefined) {
...
export function computeCvAiSurfaceHorizontalLeft({
  surfaceWidth,
  boundsLeft,
  boundsRight,
  preferredCenter,
...
function resultMinimumHeight(input: CvAiSurfacePositionInput): number {
...
function choosePlacement(args: {
  input: CvAiSurfacePositionInput;
  bounds: Bounds;
  ...
  surfaceWidth: number;
  gap: number;
}): Exclude<CvAiSurfacePlacement, "sheet"> {
  ...
  const canPlace = (placement: CvAiSurfacePlacement) => {
...
export function computeCvAiSurfacePlacement(
  input: CvAiSurfacePositionInput,
): CvAiSurfacePosition {
  ...
  const leftForVerticalPlacement =
    preferredSurfaceCenterX !== null
      ...
      : computeCvAiSurfaceHorizontalLeft({
          surfaceWidth,
...
function firstVisibleRect(selector: string): CvAiSurfaceRect | null {
  ...
  for (const element of Array.from(document.querySelectorAll<HTMLElement>(selector))) {
    ...
    return cvAiSurfaceRectFromDom(rect, window.scrollX, window.scrollY);
  }
  return null;
}

function leftDrawerRect(): CvAiSurfaceRect | null {
  if (typeof document === "undefined" || typeof window === "undefined") {
    return null;
  }
...
function viewportRect(): CvAiSurfaceRect {
  return {
    left: window.scrollX,
    top: window.scrollY,
...
export function useCvAiSurfacePosition({
    anchor,
    desiredSurfaceSize,
    mode,
  ...
  return React.useMemo(() => {
    ...
    return computeCvAiSurfacePlacement({
      anchor,
