...
describe("v1 normalization - representative blocks and pruning", () => {
  ...
  it("creates or repurposes exactly one representative block per Experience item and prunes invalid linked blocks", () => {
    ...
    const structuredItems = Array.isArray(exp.structuredContent) ? exp.structuredContent : [];

    const linkedBlocks = blocks.filter((b: any) => {
      const linked = b?.attributes?.linkedStructuredId;
      return typeof linked === "string" && linked.trim().length > 0;
    });

    // One linked block per structured id, regardless of additional free-form blocks
    expect(linkedBlocks).toHaveLength(structuredItems.length);
    const linkedIdSet = new Set(linkedBlocks.map((b: any) => b.attributes.linkedStructuredId));
    const expectedIds = structuredItems
      .map((item: any) => (typeof item?.id === "string" ? item.id : undefined))
      .filter((id: any): id is string => typeof id === "string" && id.trim().length > 0);
    expect(linkedIdSet).toEqual(new Set(expectedIds));

    // Preserve any remaining unlinked blocks (user-authored notes)
    const unlinkedBlocks = blocks.filter((b: any) => !b?.attributes?.linkedStructuredId);
    expect(unlinkedBlocks.length).toBeGreaterThanOrEqual(1);
